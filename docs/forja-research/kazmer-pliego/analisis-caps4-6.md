...
- **Numeración: `A-01 … A-78` son LOCALES a este tomo, y hay colisión conocida entre tomos.** Estado
  al cierre: `analisis-caps1-3.md` ocupa A-01…A-59, `analisis-caps10-13.md` ocupa A-60…A-143, y tanto
  `analisis-caps7-9.md` (A-01…A-71) como éste reiniciaron en A-01. Al fusionar hay que **renumerar en
  orden de libro** (caps. 1-3 → 4-6 → 7-9 → 10-13) y reescribir las referencias cruzadas internas.
  Dentro de este documento todas las referencias `←A-nn` son consistentes entre sí.
...
3. **El sesgo declarado del método.** *"all the models always over predicted the filling pressures"*
   ...
   y por qué. Eso convierte el contraste contra simulación (A-48, Tabla 5.1) en parte del método,
   no en un extra.

---

# CAPÍTULO 4 — ARQUITECTURA DEL MOLDE · A-01 … A-22
...
### A-01 · Dirección de apertura del molde
...
- **DECIDE** — apertura axial simple (dos mitades) vs *split cavity* (§13.9.1, fuera de este tomo)
  vs apertura oblicua. Determina qué es siquiera posible en A-02 y A-03.
...
- **INVALIDA** — que la línea de partición resultante (A-02) caiga en zona visible o funcional: el
...
### A-02 · Ubicación de la línea de partición — prueba de visibilidad
- **CUÁNDO** — inmediatamente después de A-01.
- **ENTRADAS** — dirección de apertura (←A-01); geometría; **y un dato que no es geométrico: qué caras
...
- **INVALIDA** — cambio de A-01; y un cambio en la declaración de superficies críticas/estéticas de
...
### A-03 · Planaridad de la línea y complejidad del plano de partición
- **CUÁNDO** — al proyectar el plano de partición hacia afuera desde la pieza (§4.1.3), justo
  después de fijar la línea.
- **ENTRADAS** — línea de partición (←A-02); dirección de apertura (←A-01).
...
- **DECIDE** — si se acepta el costo de una partición no plana o se vuelve a A-02/A-01 a buscar una
...
- **INVALIDA** — cualquier cambio de A-01 o A-02.
...
### A-04 · Ángulo de las features de interlock del plano de partición
- **CUÁNDO** — sobre el plano de partición ya construido, antes de congelar superficies.
- **ENTRADAS** — superficie de partición (←A-03); dirección de apertura (←A-01).
...
- **INVALIDA** — cambio de A-01 o A-03.
...
### A-05 · Conteo de shut-offs contra ventanas de la pieza
- **CUÁNDO** — después del plano de partición, cerrando §4.1.
- **ENTRADAS** — geometría de la pieza (número de ventanas/aberturas); plano de partición (←A-03);
  mapa de visibilidad (←A-02).
...
- **INVALIDA** — cambio de geometría de la pieza (una ventana nueva) o de A-02/A-03.
...
### A-06 · Altura del inserto por línea de enfriamiento
...
### A-07 · Redondeo de la altura del inserto al incremento de placa (hacia ARRIBA)
- **CUÁNDO** — inmediatamente después de A-06.
- **ENTRADAS** — altura calculada (←A-06); sistema de unidades del mold base (←A-22).
...
- **INVALIDA** — cambio de A-06, o cambio del sistema de unidades del proveedor.
...
### A-08 · Altura TOTAL del core para procuración
- **CUÁNDO** — al armar la lista de materiales / cotización, en paralelo al diseño.
- **ENTRADAS** — altura de diseño del core (←A-06/A-07); altura del corazón por encima del plano de
...
### A-09 · Largo y ancho del inserto: agua contra estructura, y cuál domina
- **CUÁNDO** — junto con A-06, cerrando §4.2.
...
### A-10 · Selección del layout de cavidades
- **CUÁNDO** — abre §4.3, con los insertos dimensionados.
- **ENTRADAS** — número de cavidades; tamaño del inserto (←A-09); requisitos de gating (adelanto del
  cap. 6).
...
- **DECIDE** — la planta del molde, y con ella el layout del feed system (A-51) y el balanceo.
...
### A-11 · Relación de aspecto de la envolvente de cavidades
- **CUÁNDO** — inmediatamente después de A-10; es su gate.
- **ENTRADAS** — posiciones y tamaños de todas las cavidades (←A-10).
...
- **DECIDE** — aceptar el layout o volver a A-10 (línea → rejilla / círculo / híbrido).
- **CRITERIO** — **[LIBRO]**, cota dura: *"the **width to length ratio** of the bounding envelope
  ...
  may not be designed"*.
- **INVALIDA** — cambio de A-10 o del número de cavidades.
...
### A-12 · Área usable del plano de partición (holgura ≥ ½·D por vecino)
- **CUÁNDO** — al dimensionar el mold base (§4.3.2), con el layout ya elegido.
- **ENTRADAS** — layout de cavidades (←A-10); inventario de componentes del plano de partición:
...
### A-13 · Dimensionado del mold base (L×W, A, B, S, E, C, stack)
- **CUÁNDO** — cierre de §4.3.2, con área usable resuelta.
- **ENTRADAS** — insertos (←A-06…A-09); layout (←A-10); área usable (←A-12); profundidad de la pieza.
...
- **INVALIDA** — todo lo de A-06 a A-12. Y el gate del libro: *"any mistakes in the mold base
...
### A-14 · Compatibilidad con la máquina 1 — ancho entre tie bars
- **CUÁNDO** — §4.3.3, con el mold base especificado y ANTES de comprarlo.
- **ENTRADAS** — L×W del mold base (←A-13); **saliente de tapones de agua y conectores de hot runner**
  (←A-49 si hay colada caliente); ficha de la máquina del moldeador.
...
### A-15 · Compatibilidad con la máquina 2 — daylight (falla por los dos lados)
- **CUÁNDO** — junto con A-14.
- **ENTRADAS** — stack height del molde (←A-13); carrera de apertura necesaria (←A-72/A-74 si es de
...
- **INVALIDA** — cambio de stack (A-13) o de la carrera de apertura (A-72/A-74). Pasar de dos a tres
...
### A-16 · Compatibilidad con la máquina 3 — ventana de disparo
- **CUÁNDO** — junto con A-14/A-15.
- **ENTRADAS** — volumen de cavidades × nº de cavidades; volumen del feed system (←A-56); capacidad
...
- **CRITERIO** — **[LIBRO], banda de dos colas otra vez**: la HM320 tiene 490 cc de máximo pero
  *"this machine is **ideally suited** for molds requiring a shot volume **between 120 cc and 250
  ...
  No es "cabe / no cabe".
- **INVALIDA** — cambio del volumen del feed (A-56) o del número de cavidades.
...
### A-17 · Compatibilidad con la máquina 4 — tonelaje (suficiente y no excesivo)
- **CUÁNDO** — junto con A-14…A-16, pero **depende del cap. 5**: el número entra desde A-40.
- **ENTRADAS** — tonelaje requerido (←A-40); tonelaje de la máquina (HM320: 3200 kN = 326 t métricas).
...
- **INVALIDA** — cualquier cosa que mueva A-40: cambio de presión de cavidad, de área proyectada o del
  número de cavidades.
...
### A-18 · Difusividad térmica del acero del molde
...
- **SALIDA** — m²/s.
- **DECIDE** — qué metal se compara contra cuál; alimenta A-19 (costo de operación) y el diseño de
...
### A-19 · Factores de costo de fabricación y de operación del molde
- **CUÁNDO** — §4.4.3, comparando el conjunto de metales candidatos.
- **ENTRADAS** — costo del material; dureza Brinell; tasa de maquinado; difusividad (←A-18).
...
### A-20 · Selección del material de insertos por la matriz de Tabla 4.1
...
- **ENTRADAS** — tipo de aplicación (melt no abrasivo/presión baja · ligeramente abrasivo o presión
  ...
  (<10,000 ciclos / moderado / >1,000,000); más los factores de A-19 como desempate.
...
### A-21 · Compatibilidad del material elegido con el catálogo de mold bases
- **CUÁNDO** — inmediatamente después de A-20, antes de emitir la orden.
- **ENTRADAS** — material del inserto (←A-20); catálogo de bases del proveedor (←A-22).
...
- **INVALIDA** — cambio de proveedor (A-22) o de material de inserto (A-20).
...
### A-22 · Calificación del proveedor de mold base
- **CUÁNDO** — §4.3.4, en paralelo a A-13, antes de la orden.
...
- **DECIDE** — a quién se le compra, y qué incrementos de placa aplican en A-07 (½″ vs 10 mm).
...
- **INVALIDA** — nada río arriba; es una entrada externa. Pero cambiarlo invalida A-07 y A-13.
- **¿TENEMOS?** — **FALTA.** No hay modelo de proveedor ni SLA en el repo; el catálogo
  `STANDARD_BASES` es genérico y sin dueño.

---

# CAPÍTULO 5 — ANÁLISIS DE LLENADO · A-23 … A-48
...
>
> **§5.3 es la caja de herramientas** (A-24…A-28): cuatro modelos reológicos que no se corren "en un
> orden", se ELIGEN. Kazmer los presenta de menor a mayor fidelidad y declara cuál usa dónde.

### A-23 · Temperatura de melt de análisis
...
### A-24 · Tasa de corte y esfuerzo cortante nominales
...
- **EL CÁLCULO** — `gammadot = v / h` (**Ec 5.2**), `tau = eta * gammadot` (**Ec 5.1**),
  ...
  de 0.2 × 0.1 m.
- **SALIDA** — 1/s, Pa, N.
- **DECIDE** — establece la magnitud del problema; es la base de A-25 y del criterio de shear de los
...
### A-25 · Gradiente de presión por esfuerzo de pared
- **CUÁNDO** — §5.3.2, inmediatamente después de A-24.
- **ENTRADAS** — esfuerzo cortante en la pared (←A-24); espesor del canal.
...
### A-26 · Curva de viscosidad Cross-WLF
...
- **EL CÁLCULO** — `eta(gammadot, T, P)` — **Ecs 5.8 a 5.12**, con tres asas: `eta0`, `tau*` y `n`
  ...
  10 y 10,000 1/s.
- **SALIDA** — Pa·s.
- **DECIDE** — de dónde salen la `k` y la `n` del power law (A-28) para la ventana de shear de ESTA
...
- **¿TENEMOS?** — **FALTA.** Cero ocurrencias de Cross-WLF, `tauStar`, `D1`, `A1`, `A2` en todo
  ...
  de la viscosidad con la temperatura, y toda la palanca de A-23 (mid-range para dejar margen) queda
  sin efecto numérico.

### A-27 · Modelo newtoniano de canal rectangular
- **CUÁNDO** — §5.3.4; primera opción de cálculo de ΔP, la simple.
- **ENTRADAS** — viscosidad `mu` evaluada a la tasa de corte representativa (←A-26); geometría del
...
### A-28 · Modelo power law
- **CUÁNDO** — §5.3.5; es el modelo que el libro efectivamente usa en el ejemplo del bezel.
- **ENTRADAS** — `k` y `n` ajustados a la ventana de shear de la pieza (←A-26); geometría; caudal.
...
- **SALIDA** — Pa.
- **DECIDE** — el número de presión que se compara contra el techo (A-35).
- **CRITERIO** — **[LIBRO], sobre el ajuste**: *"the power law model can **purposefully fit to a
  ...
  general.
- **INVALIDA** — cambio de resina, de temperatura de análisis (A-23) o de la ventana de shear.
...
### A-29 · Velocidad de llenado recomendada, con lazo de convergencia
- **CUÁNDO** — §5.5.1, el primer cálculo "vivo" del capítulo.
- **ENTRADAS** — `T_melt` (←A-23), `T_wall`, conductividad `kappa` de la resina, y **la viscosidad,
  que a su vez depende de la velocidad que se está buscando** (←A-28). Ése es el lazo.
...
- **INVALIDA** — cambio de A-23, de resina, o del espesor de pared.
...
### A-30 · Tiempo de llenado y caudal
- **CUÁNDO** — inmediatamente después de A-29.
- **ENTRADAS** — velocidad convergida (←A-29); longitud de flujo (←A-43); volumen de la cavidad.
...
- **INVALIDA** — cambio de A-29 o de la longitud de flujo.
- **¿TENEMOS?** — **PARCIAL, y las dos mitades están descableadas.** `t_fill = L/v` existe **inline y
  ...
  convergido de A-29. O sea: calculamos la velocidad correcta y luego el feed system la ignora.

### A-31 · Check de banda de velocidad lineal
- **CUÁNDO** — cierre de §5.5.1.
- **ENTRADAS** — velocidad convergida (←A-29).
- **EL CÁLCULO** — comparación contra banda.
- **SALIDA** — semáforo.
- **DECIDE** — si el resultado de A-29 es creíble o hay un error de datos aguas arriba.
...
### A-32 · Construcción y segmentación del lay-flat
- **CUÁNDO** — §5.5.2, apenas elegida la ubicación del gate.
- **ENTRADAS** — geometría 3D de la pieza; ubicación de gates (←A-33).
...
- **DECIDE** — la geometría 1D sobre la que se calcula la presión (A-34) y el patrón de llenado (A-41).
...
- **INVALIDA** — mover el gate (A-33) o cambiar espesores (A-46).
...
### A-33 · Ubicación de gates para balancear el flujo
- **CUÁNDO** — **antes** del cálculo de presión, no después. Es el primer paso de §5.5.2.
- **ENTRADAS** — lay-flat (←A-32, relación circular deliberada: se itera); geometría.
...
### A-34 · Presión de llenado de la cavidad
- **CUÁNDO** — §5.5.2, con el lay-flat segmentado.
- **ENTRADAS** — segmentos (←A-32); modelo reológico (←A-27 o A-28); velocidad/caudal (←A-29/A-30);
  temperatura de análisis (←A-23).
...
- **SALIDA** — MPa.
- **DECIDE** — si la pieza es moldeable como está; y es la base del espesor mínimo (A-36) y del
  tonelaje (A-38).
- **CRITERIO** — el suyo propio es **[COMPARA]** contra el techo de A-35. El libro insiste en mostrar
...
### A-35 · Check de la banda de presión de cavidad (techo Y cola baja)
- **CUÁNDO** — inmediatamente después de A-34.
- **ENTRADAS** — presión de llenado (←A-34).
...
- **INVALIDA** — cambio de A-34, o que el moldeador declare una máquina de alta presión / un hot runner
  de baja caída.
...
### A-36 · Espesor mínimo de pared por la curva P(h)
- **CUÁNDO** — §5.5.2, cerrando el bloque de presión.
- **ENTRADAS** — el modelo de presión (←A-34) barrido sobre el espesor; el techo de presión (←A-35);
  **y el presupuesto de ΔP del feed system (←A-52)**.
...
- **CRITERIO** — **[COMPARA]**: intersección de curva con línea. El umbral es el de A-35.
...
### A-37 · Área proyectada de la cavidad
- **CUÁNDO** — §5.5.3, antes de cualquier cálculo de tonelaje.
- **ENTRADAS** — geometría de la pieza; dirección de apertura (←A-01).
...
- **DECIDE** — el multiplicando del tonelaje (A-38, A-39).
...
- **INVALIDA** — cambio de A-01 o de geometría.
...
### A-38 · Tonelaje al final del llenado
- **CUÁNDO** — §5.5.3.
- **ENTRADAS** — presión de llenado (←A-34); área proyectada (←A-37).
...
- **DECIDE** — junto con A-39, el tonelaje de la máquina (A-17).
- **CRITERIO** — **ninguno propio**; su criterio es la comparación de A-40.
- **INVALIDA** — cambio de A-34 o A-37.
...
### A-39 · Tonelaje al inicio del empaque, con piso de 50 MPa
- **CUÁNDO** — §5.5.3, en paralelo a A-38.
- **ENTRADAS** — presión de empaque (del moldeador, o el piso); área proyectada (←A-37).
...
### A-40 · Selección del tonelaje gobernante
- **CUÁNDO** — cierre de §5.5.3.
- **ENTRADAS** — tonelaje de llenado (←A-38) y de empaque (←A-39).
- **EL CÁLCULO** — `F_diseño = max(F_llenado, F_empaque)`.
- **SALIDA** — toneladas métricas.
- **DECIDE** — **la máquina** (entra directo a A-17).
...
- **INVALIDA** — cambio de A-38 o A-39.
...
### A-41 · Predicción del patrón de llenado (arcos y phantom gates)
- **CUÁNDO** — §5.5.4, con el lay-flat y el gate ya fijos.
- **ENTRADAS** — lay-flat (←A-32); gate (←A-33); velocidad lineal (←A-29).
...
- **DECIDE** — es la materia prima de A-42, A-44 y A-45.
...
### A-42 · Detección de race-tracking
- **CUÁNDO** — §5.5.4, sobre el patrón de llenado.
- **ENTRADAS** — longitudes de flujo por perímetro y por centro (←A-43); geometría (profundidad y
  ancho).
...
### A-43 · Longitudes de flujo comparadas
- **CUÁNDO** — §5.5.5, sobre el lay-flat.
- **ENTRADAS** — lay-flat (←A-32); gate (←A-33).
...
- **SALIDA** — mm por ruta.
- **DECIDE** — alimenta A-42 (detección) y A-46 (el espesor del flow leader), y A-30 (t_fill).
...
### A-44 · Localización de líneas de soldadura
- **CUÁNDO** — §5.2.3 / §5.5.4, sobre el patrón de llenado.
- **ENTRADAS** — patrón de llenado (←A-41); mapa de superficies estética o estructuralmente críticas.
...
### A-45 · Trampas de gas y última zona en llenar
- **CUÁNDO** — §5.2.3 y §5.5.4, sobre el patrón de llenado.
- **ENTRADAS** — patrón de llenado (←A-41).
...
- **INVALIDA** — cambio de gate, de espesores (A-46) o de geometría.
...
### A-46 · Dimensionado del flow leader
- **CUÁNDO** — §5.5.5, **y solo como último recurso**.
- **ENTRADAS** — longitudes de flujo (←A-43); espesor nominal; viscosidades de cada zona.
...
- **INVALIDA** — cambio de gate o de geometría; y cualquier cosa que mueva A-43.
...
### A-47 · Re-verificación del remedio (lo que costó el flow leader)
- **CUÁNDO** — después de aplicar A-46. **Es un análisis, no un trámite**: el libro lo cuantifica.
- **ENTRADAS** — geometría remediada (←A-46); el mismo modelo de presión (←A-34).
- **EL CÁLCULO** — recorrer A-34 y A-41 con los espesores nuevos y comparar contra los viejos.
...
- **DECIDE** — aceptar el remedio o buscar otro. **El fix nunca es gratis, y el libro obliga a ponerle
  precio.**
- **CRITERIO** — **[COMPARA]** contra el techo de A-35: si el +10% te saca de los 100 MPa, el remedio
  no sirve.
- **INVALIDA** — cambio de A-46.
...
### A-48 · Contraste análisis-a-mano contra simulación (Tabla 5.1)
- **CUÁNDO** — al cerrar el cap. 5, cuando existe una simulación. **No es un extra: es la calibración
  declarada del método.**
- **ENTRADAS** — todos los resultados a mano (A-29, A-34, A-38, A-39) y los de la simulación.
...
# CAPÍTULO 6 — SISTEMA DE ALIMENTACIÓN · A-49 … A-78
...
>
> Y el cap. 6 **depende del cap. 5**: el caudal con el que se dimensiona todo sale de A-30.

### A-49 · Selección del tipo de feed system
...
- **INVALIDA** — cambio del número de cavidades, del layout (A-51) o de la capacidad declarada del
  moldeador.
...
### A-50 · Comparación económica y de plazo de la arquitectura
- **CUÁNDO** — junto con A-49, cuando el tipo NO viene especificado.
...
### A-51 · Selección del layout del feed system
- **CUÁNDO** — §6.4.2, con el tipo elegido y el layout de cavidades de A-10.
- **ENTRADAS** — layout de cavidades (←A-10); tipo de feed (←A-49); número de cavidades.
...
- **DECIDE** — la geometría sobre la que corren A-55 a A-58, y si el balanceo será natural o artificial.
...
- **INVALIDA** — cambio de A-10 o de A-49.
...
### A-52 · Presupuesto de caída de presión del feed system
...
- **SALIDA** — MPa.
- **DECIDE** — el radio de cada segmento (A-58), y por rebote el **espesor mínimo de pared** (A-36).
...
- **INVALIDA** — que llegue el dato real de la máquina; o que cambie A-34.
...
### A-53 · Presupuesto de volumen del feed system
- **CUÁNDO** — §6.2.3, junto con A-52. Es la otra mitad del lazo.
- **ENTRADAS** — volumen de las cavidades; % de regrind permitido por el cliente; tipo de feed (←A-49).
- **EL CÁLCULO** — `V_feed_max = %_regrind * V_cavidades` para cold runner; para hot runner el criterio
  ...
  30% ⇒ feed ≤ 15 cc.
- **SALIDA** — cc.
- **DECIDE** — el piso de los diámetros (junto con A-52, que pone el techo).
...
### A-55 · Caída de presión por segmento del feed
- **CUÁNDO** — §6.4.3, el corazón del capítulo.
- **ENTRADAS** — geometría del segmento (L, R); caudal (←A-30); reología (←A-26/A-28).
...
- **DECIDE** — si el diseño cumple A-52; si no, dispara el lazo de diámetros.
- **CRITERIO** — **[COMPARA]** contra el presupuesto de A-52.
- **INVALIDA** — cambio de caudal (A-30), de resina, de geometría o de layout (A-51).
...
### A-56 · Volumen total del feed system
- **CUÁNDO** — §6.4.4, con la geometría propuesta.
- **ENTRADAS** — longitudes y radios de cada segmento (←A-51, A-58).
- **EL CÁLCULO** — `V_total = suma_j( N_j * L_j * pi * R_j^2 )` — **Ec 6.6**.
  ...
  27.5 cc.
- **SALIDA** — cc.
- **DECIDE** — si cumple A-53; y alimenta el volumen de disparo de A-16.
- **CRITERIO** — **[COMPARA]** contra A-53, **y la lectura depende del tipo**: esos 37 cc son
...
- **INVALIDA** — cambio de diámetros (A-58, A-67, A-68) o de layout.
- **¿TENEMOS?** — **SÍ.** `src/forja/mold/feed.ts :: feedVolume` = `Σ (count ?? 1)·L·π·R²`, Ec 6.6
  exacta.

### A-57 · Reparto del presupuesto de ΔP por longitud
- **CUÁNDO** — §6.4.5, justo antes de resolver los radios.
- **ENTRADAS** — presupuesto total (←A-52); longitud de cada segmento (←A-51).
...
- **DECIDE** — la restricción individual que resuelve A-58.
...
- **INVALIDA** — cambio de longitudes (A-51) o del presupuesto (A-52).
...
### A-58 · Solver de radio por restricción de presión
- **CUÁNDO** — §6.4.5, el paso que produce las cotas.
- **ENTRADAS** — ΔP asignado al segmento (←A-57); longitud; caudal (←A-30); reología.
...
- **DECIDE** — las cotas del feed system, que luego se redondean (A-67) y se ajustan steel-safe (A-68).
...
- **INVALIDA** — cambio de A-57, A-52, A-30 o de resina.
...
### A-59 · Barrido volumen contra presupuesto de presión
- **CUÁNDO** — §6.4.5, después del primer solver, para explorar el trade-off.
- **ENTRADAS** — el solver completo (←A-58) corrido a varios `ΔP_max`, por caudal.
...
- **CRITERIO** — **[JUICIO] informado por dos umbrales** (A-52 y A-53): el diseño válido es cualquier
...
### A-60 · Balanceo artificial de un molde familia
- **CUÁNDO** — §6.4.6, cuando las cavidades NO son iguales.
- **ENTRADAS** — volumen y presión de cavidad de cada rama (←A-34); tiempo de llenado objetivo común.
...
### A-61 · Verificación de cierre del balanceo
- **CUÁNDO** — después de A-60 (o de cualquier layout que se declare balanceado).
- **ENTRADAS** — geometría final del feed (←A-58/A-60); presiones de cavidad (←A-34).
...
- **INVALIDA** — cualquier cambio de diámetro, incluido el redondeo de A-67/A-68.
...
### A-62 · Métrica de regrind y su lectura económica
- **CUÁNDO** — §6.4.6, con el volumen del feed ya fijo.
- **ENTRADAS** — volumen del sprue y de los runners (←A-56); volumen de las cavidades.
...
- **CRITERIO** — **[COMPARA]** contra el límite del cliente (A-53) por arriba, y contra el costo de
...
### A-63 · Tiempo de enfriamiento del feed contra el de la cavidad
...
- **INVALIDA** — cambio de diámetro del sprue (que a su vez sube ΔP: retorno a A-55).
...
### A-64 · Vueltas y tiempo de residencia del hot runner
- **CUÁNDO** — §6.4.8, último sub-paso, solo si hay colada caliente.
- **ENTRADAS** — volumen del hot runner (←A-56); volumen de cavidades; tiempo de ciclo.
...
### A-65 · Diámetro hidráulico y eficiencia de la sección del runner
...
### A-66 · Sección anular del valve gate
...
- **CRITERIO** — **[COMPARA]** contra el presupuesto de A-52. El libro no da umbral propio para la
...
### A-67 · Redondeo a diámetro de cortador estándar
- **CUÁNDO** — §6.5.4, después del solver.
- **ENTRADAS** — radios calculados (←A-58); catálogo de herramienta.
...
### A-68 · Ajuste steel-safe (redondear HACIA ABAJO)
- **CUÁNDO** — §6.5.5, después de A-67. **Es un análisis distinto, no el mismo redondeo.**
- **ENTRADAS** — el diámetro estándar más cercano (←A-67).
...
### A-69 · Re-verificación post-redondeo
- **CUÁNDO** — inmediatamente después de A-67 y A-68.
- **ENTRADAS** — los diámetros finales.
- **EL CÁLCULO** — recorrer A-55 (ΔP), A-56 (volumen) y A-61 (balanceo) con las cotas redondeadas.
...
- **CRITERIO** — **[COMPARA]** contra A-52, A-53 y A-63 otra vez.
...
### A-70 · Monotonía de diámetros aguas abajo
- **CUÁNDO** — auditoría del árbol de colada, después de fijar diámetros.
- **ENTRADAS** — todos los diámetros del árbol (←A-58, A-67, A-68).
...
### A-71 · Orificio de boquilla contra entrada del sprue
...
- **CRITERIO** — **[LIBRO], y es LA excepción a A-70**: *"the **diameter of the molding machine's nozzle
...
### A-72 · Velocidad y tiempo de apertura del molde
- **CUÁNDO** — §6.3.2, al comparar dos contra tres placas.
- **ENTRADAS** — tonelaje de cierre de la máquina (←A-40/A-17); distancia de apertura.
...
- **SALIDA** — mm/s y s.
- **DECIDE** — alimenta la comparación de A-73 y el tiempo de ciclo.
...
### A-73 · Comparativa dimensional dos placas contra tres placas
- **CUÁNDO** — §6.3.2, como entregable de decisión.
- **ENTRADAS** — geometría del molde en las dos arquitecturas; velocidad de apertura (←A-72).
...
### A-74 · Apertura A–B y longitudes libres de los stripper bolts
...
- **SALIDA** — mm.
- **DECIDE** — la carrera del molde, que entra a A-15 (daylight) y a A-73.
...
### A-75 · Dimensionado de sucker pins
...
### A-76 · Longitud total y número de ramas del feed
- **CUÁNDO** — §6.4.2, como criterio de puntuación del layout.
- **ENTRADAS** — el árbol de colada (←A-51).
...
- **INVALIDA** — cambio del layout de cavidades (A-10).
...
### A-77 · La regla de velocidad constante, como CONTRAEJEMPLO
...
- **CRITERIO** — **[LIBRO], rechazo explícito**: *"While this design guideline [D_down = D_up/sqrt(n)]
  ...
  el solver por restricción de A-57 + A-58.
- **INVALIDA** — n/a.
- **¿TENEMOS?** — **SÍ, PERO AL REVÉS: está implementada y tratada como VÁLIDA.**
  ...
  layouts dimensionan con la regla que Kazmer llama inferior, en vez de con A-57/A-58. Es el hallazgo
  más accionable de este tomo.

### A-78 · Aislamiento térmico del hot runner
...
```
                        ...
                        │  A-18 alpha ──► A-19 f_making/f_operating ──► A-20 Tabla 4.1 ──► A-21 base │
                        │       A-22 proveedor ──────────────────────────────┴──► (A-07, A-13)       │
                        └───────────────────────────────────────────────────────────────────────────┘

A-01 apertura ──► A-02 parting line ──► A-03 parting plane ──► A-04 interlocks >=5°
   │                    │                      └──► A-05 shut-offs = ventanas
   │                    │
   │                    └──► A-06 altura inserto (3·D agua) ──► A-07 redondeo ARRIBA ──► A-08 H compra
   │                         A-09 L/W inserto (agua vs cheek)
   │                                   │
   │                                   ▼
   │                         A-10 layout cavidades ──► A-11 aspecto <2:1 ──► A-12 area usable (½·D)
   │                                                                              │
   │                                                                              ▼
   │                                                                     A-13 SPEC DEL MOLD BASE
   │                                                                     ═══ GATE DE COMPRA ═══
   │                                                                              │
   │                                        ┌─────────────┬───────────────┬───────┴───────┐
   │                                        ▼             ▼               ▼               ▼
   │                                   A-14 tie bars  A-15 daylight  A-16 disparo    A-17 tonelaje
   │                                                       ▲                              ▲
   └──► A-37 area proyectada ─────────────────────────────┼──────────────────────────────┤
                                                           │                              │
CAP. 5                                                     │                              │
A-23 T_melt ──► A-26 Cross-WLF ──► A-27 newtoniano / A-28 power law                       │
                                          │                                               │
                                          ▼                                               │
              A-24 gammadot/tau ─► A-25 dP/dL      A-29 v recomendada  ⟲ LAZO v-gammadot-mu│
                                                          │                               │
                                                          ├──► A-31 banda 0.01–1 m/s       │
                                                          ▼                               │
                                              A-30 t_fill y CAUDAL ──────────► (CAP. 6)    │
                                                          │                               │
        A-33 gates ⇄ A-32 lay-flat ─────────────────────► A-34 presion de llenado          │
                    │                                      │                               │
                    │                                      ├──► A-35 banda 100 MPa / cola baja
                    │                                      │              │                │
                    │                                      │              ▼                │
                    │                                      │      A-36 ESPESOR MINIMO ◄────┼── A-52
                    │                                      │                               │
                    │                                      ├──► A-38 tonelaje llenado ──┐   │
                    │                          A-37 ───────┴──► A-39 tonelaje empaque ──┼──►A-40 max ─┘
                    │                                                                   │
                    ▼
        A-41 patron de llenado ──┬──► A-42 race-tracking ◄── A-43 longitudes de flujo
                                 ├──► A-44 weld lines
                                 └──► A-45 gas traps / last-to-fill ──► (CAP. 8 venteo, CAP. 11 expulsion)
                                          │
                     A-42/A-45 ──► A-46 flow leaders ──► A-47 re-verificacion (+10% P, -10% peso)

        A-48 CONTRASTE vs SIMULACION (Tabla 5.1) ─── valida A-29, A-34, A-38, A-39, A-40

CAP. 6
A-50 economia ──► A-49 TIPO de feed ──► A-51 LAYOUT ──► A-76 longitud/ramas
                       │                    │
                       │                    ├──► A-72 v apertura ──► A-73 2 vs 3 placas ──► A-15
                       │                    │         A-74 apertura A–B (3 placas) ───────► A-15
                       │                    │
   A-34 ──► A-52 PRESUPUESTO ΔP ──► A-57 reparto por longitud ──► A-58 SOLVER DE RADIO ──► A-59 barrido
                       ▲                                                │
   A-53 PRESUPUESTO VOLUMEN ◄── A-56 volumen del feed ◄─────────────────┤
                       │                                                │
   A-30 caudal ──► A-54 Re<2300 ──► A-55 ΔP por segmento ◄──────────────┤
                                          ▲                             │
                       A-65 Dh / eficiencia de seccion ─────────────────┤
                       A-66 seccion anular ────────────────────────────┘
                                                                        │
   A-60 balanceo familia ──► A-61 cierre del balanceo ◄─────────────────┤
   A-56 ──► A-62 regrind        A-56 ──► A-64 n_turns / residencia      │
   A-58 ──► A-63 t_c feed vs cavidad                                    │
                                                                        ▼
                      A-67 cortador estandar ──► A-68 STEEL-SAFE (abajo) ──► A-69 RE-VERIFICAR
                                                                        │
                      A-70 monotonia de diametros · A-71 boquilla < sprue · A-75 sucker pins
                      A-77 regla sqrt(n) = CONTRAEJEMPLO (no produce nada) · A-78 aislamiento HR
```
...
1. `A-02 ◄══ A-01` — si la línea de partición ideal cae en zona visible o funcional, se cambia la
   **dirección de apertura** (split cavity / oblicua) para poder moverla. §4.1.1
2. `A-03 ◄══ A-02` — feature fuera de plano ⇒ rehacer el plano con superficies lofted y draft, e
   inclinar los interlocks ≥5°. §4.1.3
3. `A-11 ◄══ A-10` — aspecto ≥2:1 ⇒ cambiar el layout (línea → rejilla / círculo / híbrido). §4.3.1
4. `A-12 ◄══ A-13 / A-10` — conflicto con leader pins, return pins, SHCS, agua o pilares ⇒ **crecer la
   base** o reacomodar el layout. *"mold bases are often sized larger than what would first be
   considered"*. §4.3.2
5. `A-14…A-17 ◄══ A-13 / A-10 / máquina` — cualquiera de los cuatro semáforos en rojo tira la base, la
   cavitación o la máquina. §4.3.3
6. `A-09 ◄══ (cap. 9 y cap. 12)` — **retorno declarado y diferido**: las dimensiones L/W del inserto
   pueden REDUCIRSE después *"supported by later engineering analysis"*. Hasta entonces son
   **provisionales**, y si cambian invalidan A-13. §4.2.3

**Dentro del cap. 5**

7. `A-29 ⟲ A-29` — **lazo interno**: velocidad → tasa de corte → viscosidad → velocidad,
   *"until the velocity converges"*. 0.5 → 0.69 → 0.77 → 0.80 → 0.82 m/s. §5.5.1
8. `A-35 ◄══ A-34 / A-28 / A-23 / geometría` — la **escalera de remedios**, en el orden del libro:
...
9. `A-35(cola baja) ◄══ geometría` — presión demasiado baja ⇒ **adelgazar la pared nominal y poner
   costillas**. El remedio de la cola baja NO es el mismo que el de la cola alta. §5.1
10. `A-42 ══► A-33 ══► A-49 ══► A-46` — **escalera ordenada anti gas-trap**: (1) mover el gate al centro
...
11. `A-47 ◄══ A-34 / A-35` — el flow leader costó +10% de presión: hay que re-checar contra el techo.
    **El fix nunca es gratis.** §5.5.5
12. `A-48 ══► documenta el sesgo` — la simulación no invalida el análisis a mano (7% en presión y
...
13. `A-36 ◄══ A-52` — **el acoplamiento declarado más caro del tomo**: *"the minimum wall thickness is
...
14. `A-17 ◄══ A-40` — el tonelaje del cap. 5 entra al semáforo de máquina del cap. 4. El cap. 4 no
    cierra sin el cap. 5.
15. `A-15 ◄══ A-73 / A-74` — pasar de dos a tres placas dispara el daylight de 339 a 558 mm en el
    ejemplo del libro: el cap. 6 puede tirar la máquina elegida en el cap. 4. §6.3.2
16. `A-16 ◄══ A-56` — el volumen del feed entra al volumen de disparo, y puede sacar al molde de la
    ventana cómoda de la máquina. §4.3.3 + §6.4.4
17. `A-06 / A-09 ◄══ (cap. 9)` — el diámetro de la línea de agua es del cap. 9 y aquí se anticipa:
    cambiarlo re-dimensiona los insertos y con ellos la base.

**Dentro del cap. 6**

18. `A-55 ⟷ A-56` — **el lazo central del capítulo** (Fig 6.3): si ΔP > máximo, agrandar diámetros;
...
19. `A-63 ══► A-58 ══► A-55 ══► A-60/A-61` — si el enfriamiento del sprue estira el ciclo, se reduce el
    diámetro del sprue *"albeit with a higher pressure drop"*, lo que sube ΔP y puede exigir
    re-balancear. §6.4.7
20. `A-69 ◄══ A-67 / A-68` — **redondear no exime de re-checar**: tras el estándar y el steel-safe hay
    que recorrer ΔP, volumen y balanceo otra vez. §6.5.4
    Y dos más de la misma familia: `A-62 ══► A-52 o A-49` (si el regrind sale alto, el diseñador
    *"may assess a higher pressure drop"* o *"may wish to recommend a hot runner system"*, §6.4.6); y
    `A-51 ══► A-49` (si las cavidades del dos placas estorban el ruteo deseado, se cambia el tipo de
    molde, §6.4.2).

## Qué corre en paralelo

- **La rama de materiales (A-18 → A-19 → A-20 → A-21) es independiente de toda la geometría.** Puede
  correr desde el día 1 y solo se cruza con el resto en A-13 (catálogo de la base) y A-07 (incrementos
...
- **A-22 (proveedor)** es entrada externa: no depende de nada, pero al cambiar invalida A-07, A-13 y
  A-21.
- **A-37 (área proyectada)** solo necesita A-01. Se puede tener listo mucho antes que la presión.
- **A-24, A-25, A-26 son caja de herramientas, no pasos.** Se preparan una vez por resina y se
  reutilizan en A-27, A-28, A-55 y A-58.
- **En el cap. 6 hay dos ramas casi independientes**: la **hidráulica** (A-52 → A-59, A-63 → A-69) y la
  **de arquitectura de apertura** (A-72, A-73, A-74). Solo se encuentran en A-15 (daylight) y en el
  tiempo de ciclo.
- **A-70, A-71, A-75, A-76, A-78 son auditorías de cierre**: corren al final, sobre el diseño ya
  resuelto, y ninguna alimenta a otra.
- **A-04 y A-05** pueden correr en paralelo: los dos dependen de A-03 y ninguno del otro.
...
expense**"*. Traducido al grafo: **A-13 no se libera hasta que A-10, A-11, A-12 y los cuatro semáforos
A-14…A-17 estén en verde** — y A-17 depende de A-40, que está hasta el final del cap. 5. En la
...
|---|---|---|---|---|---|
| 4 · Arquitectura | A-01 … A-22 (22) | 10 | 9 | 3 | 0 |
| 5 · Llenado | A-23 … A-48 (26) | 9 | 11 | 6 | 0 |
| 6 · Alimentación | A-49 … A-78 (30) | 12 | 12 | 5 | 1 |
| **Total** | **78** | **31** | **32** | **14** | **1** |

### Lo que FALTA por completo (14)

`A-08` altura total del core para compra · `A-19` factores de costo con Brinell · `A-22` proveedor ·
`A-25` gradiente dP/dL · `A-26` **Cross-WLF** · `A-36` **curva P(h) y espesor mínimo** ·
`A-40` **el max(llenado, empaque) del tonelaje** · `A-47` costo del flow leader ·
`A-48` **contraste contra simulación** · `A-59` barrido V(ΔP) · `A-60` **balanceo de molde familia** ·
`A-64` n_turns y residencia del hot runner · `A-70` monotonía de diámetros · `A-76` longitud/ramas
como criterio de layout.

### Lo que hay que arreglar antes que agregar nada (por impacto / costo)

1. **`A-77` — nuestro código dimensiona con la regla que el libro RECHAZA.**
   ...
   sustituto (A-57 + A-58) ya existe en `feed.ts`. Es el arreglo más barato de mayor impacto.
2. **`A-37` — el bug de contabilidad del área proyectada.** `dfm-mesh.ts :: projectedAreaMm2` la
...
3. **`A-39` + `A-40` — el piso de 50 MPa no muerde y el `max` no existe.** Hoy el piso es una
...
4. **`A-30` + `A-34` — la cadena del cap. 5 está descableada en dos puntos.** `convergeVelocityTraced`
...
5. **`A-26` — sin Cross-WLF, A-23 no sirve de nada.** Toda la palanca del mid-range de temperatura
...
6. **`A-75` — dos verdades en el mismo repo.** `threeplate.ts :: suckerPinDesign.depthMm = 0.8·D`
...
`A-29`. `src/forja/mold/filling.ts :: recommendedVelocity` implementa la Ec 5.23 **con raíz cuadrada**
