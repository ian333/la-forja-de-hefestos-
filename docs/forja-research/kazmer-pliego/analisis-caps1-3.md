...
> **Nota de fusión:** los IDs `A-01…A-58` son **locales a este tomo (caps. 1–3)**. El tomo de caps. 7–9
> usa la misma numeración corrida desde `A-01`. Al juntar los cuatro tomos hay que prefijar
> (`C13-A-01`, `C79-A-01`, …) o renumerar; las referencias internas de este documento son consistentes
...
### A-01 · Suficiencia del intake (¿puedo arrancar?)
...
### A-02 · Clasificación restricción-vs-salida de cada campo de producción
...
- **ENTRADAS:** los 9 campos de la Tabla 2.3 (vida de la aplicación, cantidad total, horas de moldeo/año/máquina,
  tasas mín/máx de producción, tcycle esperado, cavidades por molde, family mold sí/no + nº de partes, nº de moldes) ←A-01.
...
- **DECIDE:** el **espacio de búsqueda** de todo el cap. 3. Si `cavidades` es restricción, A-51 no barre
  cavitaciones: las fija. Si es salida, A-51 barre y A-49 decide.
- **CRITERIO:** **[JUICIO]** — lo declara el cliente; el sistema solo lo registra y lo respeta.
- **INVALIDA:** que el cliente cambie de postura (típico cuando ve el break-even de A-49: suelta la
...
### A-03 · Capacidad requerida: cavidades mínimas por throughput
- **CUÁNDO:** en cuanto hay una estimación de ciclo (A-42) y la cantidad anual; se re-corre por cada
  variante del barrido A-51.
- **ENTRADAS:** cantidad anual y horas de moldeo por año por máquina (Tabla 2.3) ←A-02 · tcycle ←A-42.
...
- **INVALIDA:** cambio de espesor (A-42 va con h²), cambio de modo de operación (semi-auto ↔ full-auto),
...
### A-04 · Auditoría de tolerancias (¿alcanzable o sobre-especificada?)
- **CUÁNDO:** al capturar la Tabla 2.6, antes de prometer nada; y otra vez si el cliente aprieta una cota.
- **ENTRADAS:** tolerancia general en % (mm/mm) + la lista de tolerancias críticas ←A-01/A-02 ·
  material y su rango de contracción (Tabla 2.10) ←A-07.
...
- **DECIDE:** tres cosas a la vez — (1) si se abre la conversación con el equipo de producto,
  ...
  **modificaciones durante el commissioning** (costo oculto que si no se cotiza, se come el margen),
  (3) el acero del inserto (A-24: tolerancia apretada empuja a acero duro).
...
- **INVALIDA:** cambio de material (otra contracción), cambio de espesor o de geometría que mueva el
  perfil de contracción, cambio de acabado (A-19).
...
### A-05 · Auditoría regulatoria y estética
...
- **ENTRADAS:** normas aplicables (ANSI, FDA I/II/III, IEC, MIL-SPEC, ISO, UL) · sistema de color
  ...
  (proveedor/número) y **superficies críticas** ←A-01.
...
- **DECIDE:** dónde NO puede ir la compuerta ni la línea de partición ni el pin expulsor (restricción que
  viaja hasta los caps. 7 y 11); el acabado objetivo (A-19); y si no hay normas, se declara
...
### A-06 · Semáforo meta-vs-estimado de costo por pieza
- **CUÁNDO:** al cerrar la primera pasada de costeo, y en cada re-cotización.
- **ENTRADAS:** las 4 metas de la Tabla 2.1 (material, molde, proceso, total por pieza) ←A-01 ·
  los 4 estimados ←A-39, A-41, A-45, A-47.
...
### A-07 · Ficha de material: verificación y documentación de supuestos
...
- **DECIDE:** toda constante física río abajo: contracción, ciclo, tasas de maquinado (A-26/A-27), costo
  de material (A-25, A-41), factor de mantenimiento (A-38).
...
- **¿TENEMOS?** **PARCIAL y bueno.** `moldbase.ts::MOLD_METALS` trae los 11 metales del Apéndice B con
  números exactos (incluye `volMachineM3h` y `areaMachineM2h`, que son justo lo que A-26/A-27 necesitan) y
...
### A-08 · Detección de conflictos entre subsistemas y arbitraje por importancia relativa
...
### A-09 · Clasificación FIRME vs DIFUSO para compra concurrente
...
porque mueven el factor de complejidad (A-28) y el ciclo (A-42).

### A-10 · Checklist DFM de 9 puntos (agregador)
- **CUÁNDO:** al cerrar la revisión de pieza, como gate de entrada al diseño del molde.
- **ENTRADAS:** los veredictos de A-12 (espesor uniforme/mínimo), A-16 (esquinas), A-14 (ribs), A-15 (bosses),
  A-18 (draft), A-20 (undercuts), A-04 (tolerancias alcanzables), más "gates especificados" y
...
- **¿TENEMOS?** **PARCIAL — 7 de 9.** `dfm.ts::checkDFM` cubre espesor, esquinas, ribs, bosses, draft y
  ...
  pero **ninguno de los dos entra al checklist**; y "tolerancias alcanzables" (A-04) falta por completo.
  El checklist de 9 puntos como objeto único con semáforo no existe.

### A-11 · Checklist DFA de 6 puntos
...
- **ENTRADAS:** el ensamble del producto (nº de partes, dirección de ensamble, snaps, sujetadores,
  simetría, requisitos de take-out) ←A-01.
...
### A-12 · Uniformidad de pared y dirección de la transición
...
- **ENTRADAS:** mapa de espesor de la pieza (o los espesores declarados) · posición de la compuerta
  (define la **dirección** del flujo) ←A-05/A-20.
...
- **DECIDE:** rediseño de la pieza (A-13), reubicación de la compuerta, o aceptar el defecto (jetting,
...
### A-13 · Trade-off espesor nominal vs pared delgada + costillas
- **CUÁNDO:** cuando A-12 reprueba, o cuando el producto pide más rigidez.
- **ENTRADAS:** espesor nominal actual · rigidez requerida ←A-01 (Tabla 2.4: carga, deflexión permisible).
- **EL CÁLCULO:** comparación de dos diseños de rigidez EQUIVALENTE, con los números del libro:
  ...
  frente a pared delgada con costillas. El +70 % de ciclo es coherente con A-42 (`t ∝ h²`: 1.30² = 1.69).
...
### A-14 · Verificación de costilla (rib)
- **CUÁNDO:** por cada costilla, en la revisión de pieza.
- **ENTRADAS:** base, altura y paso de cada costilla · espesor nominal `h` ←A-12 · si la resina está
  cargada y si la aplicación es estética ←A-05/A-07.
...
- **INVALIDA:** cambio de `h` (todas las razones se mueven), cambio de resina cargada→sin carga,
  reclasificación de la superficie a estética (A-05).
...
### A-15 · Verificación de boss y gusset
- **CUÁNDO:** por cada boss, en la revisión de pieza.
- **ENTRADAS:** espesor de boss/rib/gusset · `h` ←A-12 · torque de inserción del tornillo autorroscante
  y fuerza de pull-out esperada ←A-01 (Tabla 2.4).
...
- **INVALIDA:** cambio de `h`, cambio de tornillo, cambio de draft global (A-18).
- **¿TENEMOS?** **PARCIAL.** `dfm.ts::checkDFM` §2.3.3 verifica el 70 % de boss y gusset. **Faltan tres cosas:**
  ...
  **excepción de draft selectivo** — hoy el draft se juzga global (A-18) y un boss sin draft sería marcado
  como error, cuando el libro lo hace a propósito.

### A-16 · Verificación de esquinas (filetes y chaflanes)
- **CUÁNDO:** por cada arista, en la revisión de pieza.
- **ENTRADAS:** radio o chaflán de cada arista, con su tipo (externa/interna) · `h` ←A-12.
...
- **DECIDE:** rediseño de la arista. La esquina viva castiga por tres frentes distintos: producto
  ...
  especiales → sube A-29) y proceso — este último es el que se olvida: *"sharp corners greatly restrict the
...
### A-17 · Disponibilidad de herramienta para el radio elegido
- **CUÁNDO:** justo después de A-16, antes de congelar el radio.
- **ENTRADAS:** radios propuestos ←A-16 · catálogo de cortadores del taller.
...
- **DECIDE:** el valor final del radio en el CAD de la pieza; evita un renglón de herramienta custom en A-30.
...
### A-18 · Draft requerido por acabado y material
- **CUÁNDO:** después de elegir acabado (A-19) y antes de congelar la geometría; se re-corre si cambia el acabado.
- **ENTRADAS:** rugosidad o profundidad de textura [µm] ←A-19 · material (cargado / flexible) ←A-07 ·
...
- **DECIDE:** el ángulo de las paredes verticales, la fuerza de expulsión (cap. 11) y, si el área en rojo
  es negativa, la existencia de un undercut (A-20 mide exactamente eso).
...
- **INVALIDA:** **cambiar el acabado cambia el draft** (A-19 → A-18 es el retorno más olvidado del cap. 2);
...
- **¿TENEMOS?** **SÍ, y es el mejor implementado del cap. 2.** `dfm.ts::draftForFinish` implementa
  ...
  **mide** sobre la malla (`draft.{pctBelowMin, pctBelowTable, minDeg, tableDeg}` + mapa de draft por columna).
  Falta el ajuste direccional por material (cargado → subir, flexible → bajar) y la excepción de A-15.

### A-19 · Selección de acabado y sus TRES efectos simultáneos
- **CUÁNDO:** al fijar la estética (Tabla 2.7), antes de A-18 y antes de A-31.
- **ENTRADAS:** requisito estético del cliente (gloss, textura, superficies críticas) ←A-05.
- **EL CÁLCULO:** selección en la escala SPI (A-1 ~0.01 µm Ra … D-3 ~4 µm Ra, Tabla 2.12) o textura
  ...
  **una sola decisión propaga tres consecuencias**: (1) el **pre-acabado obligatorio** — SPI B para texturas
  ...
  (2) **+1° de draft por 20 µm** (A-18); (3) **el costo y el tiempo** de acabado (A-31).
...
### A-20 · Detección y clasificación de undercuts
...
- **EL CÁLCULO:** un undercut es **área con draft negativo respecto a la dirección de apertura** — el mismo
  render de A-18, canal rojo; el conteo de **regiones rojas conexas** da el número de mecanismos que el molde
...
- **DECIDE:** el mecanismo que arrastra cada undercut (slide o lifter externo, interno, core pull actuado)
  y por lo tanto **su coeficiente de costo** en A-36 (Tabla 3.9). El junior VE cuánto cuesta cada feature.
...
### A-21 · Decisión eliminar-vs-conservar el undercut
- **CUÁNDO:** por cada undercut detectado, antes de cotizar.
- **ENTRADAS:** el undercut y su mecanismo ←A-20 · costo del mecanismo ←A-36 · costo de la alternativa
  (operaciones post-molde, o partir la pieza en varias) ←A-11.
...
### A-22 · Propuesta de rediseño que abarata el molde del cliente
- **CUÁNDO:** al cerrar la revisión de pieza, antes de entregar la cotización.
- **ENTRADAS:** todos los hallazgos de A-10 a A-21 · el delta de costo de cada corrección ←cap. 3.
...
- **¿TENEMOS?** **FALTA.** El sistema reporta defectos DFM (`dfm.resumen`) pero nunca cuantifica cuánto
  ahorraría corregirlos, ni arma la propuesta para el cliente. Es el mismo hueco que A-13 y A-21: nos falta
...
### A-23 · Dimensiones del inserto de cavidad
- **CUÁNDO:** primer paso del costeo, en cuanto hay bbox de la pieza.
- **ENTRADAS:** Lpart, Wpart, Hpart [m] ←A-01.
...
- **DECIDE:** cuánto acero se compra por juego (A-25), cuánto hay que remover (A-26), y el tamaño de la
  base (A-34). Es el nodo del que cuelga casi todo el capítulo.
...
### A-24 · Selección del acero del inserto
- **CUÁNDO:** después de A-23, antes de A-25/A-26.
- **ENTRADAS:** tolerancia (¿apretada?) ←A-04 · volumen de producción ←A-01/A-02 · abrasividad y
  corrosividad de la resina ←A-07 · si hay pulido espejo ←A-19.
...
- **DECIDE:** costo de material (A-25), velocidad de maquinado (A-26/A-27 — un acero duro cuesta más Y se
  maquina más lento: doble castigo), y el factor de mantenimiento (A-38 — donde se paga el beneficio).
...
- **INVALIDA:** cambio de resina, de volumen o de tolerancia (A-04).
...
### A-25 · Costo de material del inserto
- **CUÁNDO:** inmediatamente después de A-23 y A-24.
- **ENTRADAS:** Vcav [m³] ←A-23 · rho, kappa del acero ←A-24.
- **EL CÁLCULO:** `C_material = Vcav · rho · kappa` [$]. (Bezel: **$435**.)
- **SALIDA:** $ por juego de cavidad.
- **DECIDE:** el primer sumando de A-32.
- **CRITERIO:** **[LIBRO]** aritmética directa.
- **INVALIDA:** A-23 o A-24.
- **¿TENEMOS?** **SÍ.** `moldcost-detailed.ts::estimateMoldCost` → `materialUSD`. Test: $435 ±3.

### A-26 · Tiempo de maquinado volumétrico
- **CUÁNDO:** con Vcav y el acero ya elegidos.
- **ENTRADAS:** Vcav [m³] ←A-23 · `R_volumen` del acero [m³/h] ←A-24 (Apéndice B).
...
- **SALIDA:** horas.
- **DECIDE:** el grueso del tiempo de maquinado (A-30).
...
### A-27 · Tiempo de maquinado por área
- **CUÁNDO:** junto con A-26 (paralelo).
- **ENTRADAS:** área superficial de la pieza [m²] ←CAD/A-01 · `R_area` del acero [m²/h] ←A-24.
...
- **SALIDA:** horas.
- **DECIDE:** el segundo sumando del tiempo de maquinado (A-30).
- **CRITERIO:** **[LIBRO]**.
...
### A-28 · Factor de complejidad de la pieza
- **CUÁNDO:** junto con A-26/A-27.
- **ENTRADAS:** área superficial [mm²] y volumen [mm³] de la pieza ←A-01 · espesor de pared `h` [mm] ←A-12.
...
- **DECIDE:** multiplica el tiempo de maquinado (A-30). Es el mecanismo por el que **cada costilla, boss y
...
### A-29 · Factor de maquinado por mezcla de procesos
...
- **DECIDE:** multiplica el tiempo de maquinado (A-30). Un 4 contra un 1 **cuadruplica** el driver más grande
  del costo del molde.
...
- **¿TENEMOS?** **PARCIAL — y aquí sustituimos un juicio del libro por una heurística nuestra.**
  ...
  es un mapeo inventado desde A-28, y encima acopla dos análisis que el libro mantiene separados
  (un cascarón grande y liso puede tener complejidad alta y cero EDM). Falta el editor de mezcla de procesos.

### A-30 · Costo de maquinado del inserto
- **CUÁNDO:** cierra el bloque de maquinado.
- **ENTRADAS:** t_vol ←A-26 · t_area ←A-27 · f_complexity ←A-28 · f_machining ←A-29 ·
...
- **SALIDA:** horas y $.
- **DECIDE:** el sumando dominante de A-32, y por lo tanto casi todo el precio del molde.
...
### A-31 · Tiempo y costo de acabado por zonas
- **CUÁNDO:** después de A-19 (acabado elegido) y del despiece de superficies.
- **ENTRADAS:** áreas por nivel de acabado [m²] ←A-19/A-05 · tasas de la Tabla 3.6
...
- **DECIDE:** el tercer sumando de A-32; y si conviene subcontratar (el pulido y el texturizado se
  subcontratan por su alto contenido de mano de obra).
- **CRITERIO:** **[LIBRO]** tabla de tasas + la regla de no doble conteo.
- **INVALIDA:** cambio de acabado (A-19) o del despiece de superficies críticas (A-05).
...
### A-32 · Costo de un juego de cavidad
- **CUÁNDO:** cierra el bloque de insertos.
- **ENTRADAS:** C_material ←A-25 · C_machining ←A-30 · C_finish ←A-31.
- **EL CÁLCULO (Ec. 3.4):** `C_cavity = C_material + C_machining + C_finish` [$].
  (Bezel: $435 + $25,800 + $1,700.)
- **SALIDA:** $ por juego (core + cavity).
- **DECIDE:** la base del escalamiento por cavitación (A-33) y, vía A-36, también la customización.
...
### A-33 · Descuento por multiplicidad de juegos de cavidad
- **CUÁNDO:** al escalar de 1 cavidad a n.
- **ENTRADAS:** C_cavity ←A-32 · nº de cavidades ←A-02/A-51.
...
- **DECIDE:** el primer término de A-37; y es una de las dos fuerzas que hacen que más cavidades **no**
  cuesten proporcionalmente más (la otra es que el ciclo se reparte, A-45).
- **CRITERIO:** **[LIBRO]** tabla explícita, sobreescribible.
- **INVALIDA:** cambio de cavitación (A-51).
...
### A-34 · Dimensiones del mold base
- **CUÁNDO:** después de A-23 y de fijar la cavitación.
- **ENTRADAS:** Lcav, Wcav, Hcav ←A-23 · n_cavidades ←A-02/A-51.
...
- **DECIDE:** la masa (A-35) y, río abajo, la compatibilidad con la máquina (tie bars, daylight).
- **CRITERIO:** **[LIBRO]**, con **sesgo conservador declarado**.
- **INVALIDA:** A-23 o la cavitación.
...
### A-35 · Masa y costo del mold base
- **CUÁNDO:** después de A-34.
- **ENTRADAS:** L, W, H del molde [m] ←A-34 · acero de base elegido (DME #1/#2/#3).
- **EL CÁLCULO (§3.3.2, Ec. 3.15 y la de costo):**
  ...
  (Bezel: 538 kg → **$3,700**.)
- **SALIDA:** kg y $.
- **DECIDE:** el segundo término de A-37; y el acero de la base (decisión separada del acero del inserto).
- **CRITERIO:** **[LIBRO]** regresión con procedencia declarada.
- **INVALIDA:** A-34; cambio de acero de base.
...
### A-36 · Costo de customización por subsistema
- **CUÁNDO:** después de A-33 y A-35, cuando ya se sabe qué tecnologías lleva el molde.
- **ENTRADAS:** C_cavities ←A-33 · C_mold_base ←A-35 · las 5 selecciones de tecnología:
  alimentación (Tabla 3.7 de feed) ←A-49/A-53 · enfriamiento (Tabla 3.8) · expulsión (Tabla 3.9) ←A-20/A-21 ·
...
- **DECIDE:** el tercer término de A-37; y en vivo, cuánto cuesta cada palomita de tecnología — es donde
  el undercut de A-20 se convierte en dinero.
...
### A-37 · Costo total del molde
- **CUÁNDO:** cierra §3.3.
- **ENTRADAS:** C_cavities ←A-33 · C_mold_base ←A-35 · C_customization ←A-36.
- **EL CÁLCULO (Ec. 3.2):** `C_total_mold = C_cavities + C_mold_base + C_customization` [$].
  (Bezel: $27,900 + $3,700 + $43,200 ≈ **$74,800**.)
- **SALIDA:** $ del molde.
- **DECIDE:** el costo fijo de cada escenario (A-48), la amortización por pieza (A-39) y el precio (A-57).
- **CRITERIO:** **[LIBRO]** suma.
...
### A-38 · Factor de mantenimiento
- **CUÁNDO:** al amortizar el molde, después de A-24 (ya se sabe el acero).
- **ENTRADAS:** dureza/tipo de molde (suave, acero estándar P20, endurecido H13) ←A-24 ·
  abrasividad del plástico (sin carga, viscoso o con partícula, con fibra) ←A-07.
...
- **DECIDE:** multiplica la amortización (A-39). Y sobre todo, **decide el acero** (A-24) si se muestra a
...
### A-39 · Costo de molde amortizado por pieza
- **CUÁNDO:** después de A-37 y A-38.
- **ENTRADAS:** C_total_mold ←A-37 · cantidad total de producción ←A-01/A-02 · f_maintenance ←A-38.
- **EL CÁLCULO (Ec. 3.20):** `C_mold/part = (C_total_mold / n_total) · f_maintenance` [$/pieza].
  (Bezel: **$0.22**.)
- **SALIDA:** $/pieza.
- **DECIDE:** el primer término de A-47; y es el término que dispara el veredicto de sobrediseño (A-54).
- **CRITERIO:** **[LIBRO]**.
- **INVALIDA:** A-37, A-38, o un cambio de la cantidad (que es justo lo que A-50 dice que no se conoce bien).
- **¿TENEMOS?** **SÍ.** `estimatePartCost` → `moldPerPart`.

### A-40 · Factor de desperdicio del feed y decisión de regrind
- **CUÁNDO:** al costear material, después de elegir arquitectura de alimentación.
- **ENTRADAS:** tipo de feed ←A-49/A-53 · política de regrind del cliente y del taller.
...
- **DECIDE:** multiplica el material (A-41). Y contiene una trampa contraintuitiva: **el hot runner no
...
### A-41 · Costo de material por pieza
- **CUÁNDO:** después de A-40.
- **ENTRADAS:** volumen de la pieza [m³] ←A-01 · rho y kappa del plástico ←A-07 · f_feed_waste ←A-40.
- **EL CÁLCULO (Ec. 3.21):** `C_material/part = V_part · rho · kappa · f_feed_waste` [$/pieza].
  (Bezel: **$0.06**.)
- **SALIDA:** $/pieza.
- **DECIDE:** el segundo término de A-47.
- **CRITERIO:** **[LIBRO]**.
- **INVALIDA:** cambio de espesor o de geometría (A-12/A-13), de resina, o de feed.
- **¿TENEMOS?** **SÍ.** `estimatePartCost` → `materialPerPart` con `PLASTICS[plastic]` y `FEED_WASTE`.

### A-42 · Tiempo de ciclo estimado
- **CUÁNDO:** temprano — lo necesita A-03 (throughput) y A-45 (costo de proceso).
- **ENTRADAS:** espesor de pared `h` [mm] ←A-12 · modo de operación y tipo de feed ←A-53.
...
- **SALIDA:** segundos.
- **DECIDE:** el costo de proceso (A-45), las cavidades mínimas (A-03) y la fecha de entrega.
- **CRITERIO:** **[LIBRO]**. La trampa más citada del capítulo: **el espesor entra AL CUADRADO** —
  3 mm no es el doble de 1.5 mm, es ~4×. Cada vez que alguien engruesa "para ganar rigidez" (A-13),
  esto es lo que paga.
...
### A-43 · Tonelaje de cierre
- **CUÁNDO:** para elegir máquina y su tarifa.
- **ENTRADAS:** n_cavidades · Lpart, Wpart (área proyectada) ←A-01.
...
- **DECIDE:** la tarifa horaria de máquina (A-44) y la selección de inyectora.
...
### A-44 · Tarifa horaria de la máquina de moldeo
- **CUÁNDO:** después de A-43.
- **ENTRADAS:** F_clamp [toneladas métricas] ←A-43 · clase de máquina y auxiliares (Tabla 3.13 de capacidad).
...
- **SALIDA:** $/h.
- **DECIDE:** el costo de proceso (A-45); y qué auxiliares se justifican.
- **CRITERIO:** **[LIBRO]** ecuación + tabla de sumadores.
- **INVALIDA:** A-43; cambio de máquina o de auxiliares.
...
### A-45 · Costo de proceso por pieza
- **CUÁNDO:** después de A-42 y A-44.
- **ENTRADAS:** t_cycle [s] ←A-42 · n_cavidades ←A-02/A-51 · R_molding [$/h] ←A-44.
- **EL CÁLCULO (Ec. 3.22):** `C_process/part = (t_cycle / n_cavities) · R_molding / 3600` [$/pieza].
  (Bezel: **$0.19**.)
- **SALIDA:** $/pieza.
- **DECIDE:** el tercer término de A-47; y es la fuerza que empuja hacia más cavidades (el ciclo se divide).
- **CRITERIO:** **[LIBRO]**.
- **INVALIDA:** A-42, A-44, o la cavitación.
- **¿TENEMOS?** **SÍ.** `estimatePartCost` → `processPerPart`.

### A-46 · Yield esperado
- **CUÁNDO:** al cerrar el costo por pieza, y otra vez en arranque de producción.
- **ENTRADAS:** volumen de producción · nivel de exigencia de calidad ←A-04/A-05.
...
- **DECIDE:** divide TODO el costo por pieza (A-47).
...
### A-47 · Costo por pieza
- **CUÁNDO:** cierra §3.4.
- **ENTRADAS:** C_mold/part ←A-39 · C_material/part ←A-41 · C_process/part ←A-45 · yield ←A-46.
...
- **DECIDE:** la comparación entre escenarios (A-48/A-49), el semáforo contra la meta del cliente (A-06)
  y el veredicto de sobrediseño (A-54).
...
### A-48 · Costo total de producción por escenario
- **CUÁNDO:** una vez por cada escenario candidato.
- **ENTRADAS:** C_total_mold ←A-37 (costo fijo) · C_part ←A-47 (costo marginal) · cantidad `n`.
...
### A-49 · Break-even entre arquitecturas
- **CUÁNDO:** después de tener al menos dos escenarios costeados.
- **ENTRADAS:** C_fixed y C_marginal de dos escenarios ←A-48.
...
- **DECIDE:** la arquitectura de alimentación y la cavitación, **en primera instancia**: es el ganador
  económico, sujeto a los vetos de A-53.
...
### A-50 · Banda de sensibilidad sobre la cantidad de producción
- **CUÁNDO:** junto con A-49, antes de presentar la recomendación.
- **ENTRADAS:** n_breakeven ←A-49 · la incertidumbre de la cantidad ←A-01/A-02.
...
### A-51 · Barrido de escenarios (la tabla comparativa)
- **CUÁNDO:** en cuanto la cavitación o el feed son "salida a optimizar" (A-02).
- **ENTRADAS:** el conjunto de arquitecturas × cavitaciones a evaluar · toda la cadena A-23…A-48.
...
- **¿TENEMOS?** **SÍ, y es de lo mejor del sistema.** `moldmachine.ts` barre
  ...
  por throughput (A-03) y ordena por `totalUSD`; `mold-contratos.ts::contratoCosto` criterio `costo-menu`
...
### A-52 · Payback del hot runner de alta cavitación
- **CUÁNDO:** cuando el ganador de A-49 es un molde caro de alta cavitación.
- **ENTRADAS:** delta de inversión ←A-37 · ahorro por pieza ←A-47 · tasa de producción ←A-03.
...
- **INVALIDA:** A-37, A-47, o cambio de la tasa de producción.
- **¿TENEMOS?** **FALTA el cálculo.** `moldmachine.ts` declara `spec.vetos.paybackMaxMeses` en el tipo,
  pero **nadie lo calcula ni lo compara** (ver A-53).

### A-53 · Vetos no económicos sobre el ganador
- **CUÁNDO:** después de A-49, antes de recomendar.
- **ENTRADAS:** ganador económico ←A-49 · restricciones no económicas del cliente y del moldeador.
...
### A-54 · Veredicto de SOBREDISEÑO (la lectura de la proporción)
- **CUÁNDO:** al final de §3.4, mirando el costo por pieza ya desglosado.
- **ENTRADAS:** los tres términos de A-47 (molde, material, proceso) en $/pieza.
...
- **DECIDE:** **RETORNO a A-51** con una arquitectura más barata. Es la señal de re-cotizar más importante
...
### A-55 · Sanity check geográfico del resultado
- **CUÁNDO:** justo antes de soltar la cotización.
- **ENTRADAS:** costo total ←A-37 · los coeficientes de mano de obra usados (tarifa de maquinado y de acabado).
...
### A-56 · Convergencia de la especificación del molde
...
- **CRITERIO:** **[JUICIO]**.
- **INVALIDA:** cualquier retorno disparado por A-54, A-53, A-06 o A-22.
- **¿TENEMOS?** **PARCIAL.** El barrido de 15 variantes (A-51) es una convergencia de una sola pasada;
  no hay bucle que reaccione al veredicto de A-54 generando y evaluando la alternativa recomendada.
  El contrato lo **detecta** y lo **reporta**, pero nadie cierra el lazo automáticamente.

### A-57 · Costo verdadero vs precio, y términos de pago
- **CUÁNDO:** al armar el documento de cotización.
- **ENTRADAS:** C_total_mold ←A-37 · C_part ←A-47 · política comercial.
...
### A-58 · Calendario de desembolsos del proyecto
- **CUÁNDO:** al presentar la cotización, para que el cliente vea el flujo de caja.
- **ENTRADAS:** términos de pago ←A-57 · duración del proyecto ←A-30 (horas de maquinado).
...
- **CRITERIO:** **[LIBRO]** la estructura; los montos salen de A-37/A-47.
...
### A-59 · Iteración inserto ↔ base discreta (frontera con el cap. 4)
- **CUÁNDO:** al aterrizar el tamaño estimado del molde en una base real.
- **ENTRADAS:** L, W, H estimados ←A-34 · catálogo de bases estándar.
...
- **DECIDE:** las cotas que se congelan (y por lo tanto lo que se puede comprar, A-09), porque
  *"these dimensions are quite expensive to change once the mold making process has begun"*.
- **CRITERIO:** **[LIBRO]** catálogo; **[JUICIO]** cuándo congelar.
- **INVALIDA:** cambio de cavitación o de tamaño de inserto — **RETORNO a A-23**.
- **¿TENEMOS?** **PARCIAL.** `moldbase.ts::selectMoldBase` aterriza en bases estándar
  ...
  achicar el inserto, nadie regresa a A-23 a re-dimensionarlo y re-costear.
...
| Toll-gates del desarrollo de producto (concepto→diseño→desarrollo→scale-up→launch) | §2.1, Fig 2.1 | Estados del proyecto, no cálculo (aunque cada gate consume las salidas de A-06/A-10) |
...
| Tip de CAD: filetear exteriores antes del shell | §2.3.4 | Técnica de modelado (aunque condiciona A-16) |
...
| Que los drivers de costo NO incluyen indirectos (se absorben en las tarifas horarias) | §3.2 | Regla contable de no-doble-conteo; gobierna A-30/A-44, no es un análisis aparte |
...
1. **Cap. 3 sin fichas visuales.** `verificaciones-visuales.md` tiene V1.1 y V2.1–V2.7, y salta directo a V4.1:
   **no hay ninguna V3.x**. El contenido gráfico de las Figs. 3.1 (calendario de desembolsos),
   ...
   3.5 (desglose del bezel) **no está**. Solo sus descripciones textuales. Afecta a A-58 sobre todo.
...
3. **El umbral de "sobrediseño" (A-54).** El libro dice *"large cost of the mold relative to…"* sin dar
   porcentaje. Nuestro 50 % es calibración propia.
4. **La ecuación de payback (A-52).** El criterio está citado; la fórmula no.
5. **La ecuación de throughput (A-03).** Los campos de la Tabla 2.3 están; la aritmética que los liga, no.
6. **Umbrales numéricos de uniformidad de pared (A-12).** El libro califica con palabras
...
|---|---|---|
| Cap. 1 | **2** | A-08 (conflictos §1.2), A-09 (firme vs difuso §1.5) |
| Cap. 2 | **20** | A-01…A-07 (worksheets e intake), A-10…A-22 (DFM §2.3) |
| Cap. 3 | **36** | A-23…A-58 |
| Cap. 4 intro | **1** | A-59 (inserto ↔ base discreta) |
...
|---|---|---|
| **SÍ (implementado y con ancla verificada)** | **26** | A-12, A-14, A-16, A-18, A-20, A-23, A-24, A-25, A-26, A-27, A-28, A-30, A-32, A-33, A-34, A-35, A-36, A-37, A-39, A-41, A-45, A-47, A-48, A-49, A-51, A-54 |
| **PARCIAL** | **21** | A-02, A-03, A-07, A-08, A-10, A-15, A-19, A-21, A-29, A-31, A-38, A-40, A-42, A-43, A-44, A-46, A-53, A-55, A-56, A-57, A-59 |
| **FALTA** | **12** | A-01, A-04, A-05, A-06, A-09, A-11, A-13, A-17, A-22, A-50, A-52, A-58 |

El patrón salta a la vista: **la aritmética del cap. 3 está casi completa (Fase 2 al 100 %), y lo que falta
es todo lo que el libro pone ALREDEDOR de la aritmética** — los worksheets que la alimentan (A-01, A-04,
A-05, A-06), las tablas de juicio que la calibran (A-38 mantenimiento, A-46 yield, A-29 mezcla de procesos)
y los retornos que la reabren (A-13, A-22, A-50, A-52). Sabemos calcular el número; nos falta saber si el
número es bueno y qué hacer cuando no lo es.

### Los 6 huecos que más duelen (por impacto sobre el número final)

1. **A-38 · matriz de mantenimiento (Tabla 3.11).** `f_maint` fijo en 3 para todo. Un molde suave con
   resina cargada de vidrio debería dar **×20**: cotizamos ~7× barato en ese caso. Costo de arreglo: una tabla 3×3.
2. **A-46 · Tabla 3.14 de yield.** `yield` fijo en 0.98 para todo. Y el yield **divide el costo completo**,
   así que el error se propaga a los tres términos. Costo de arreglo: una tabla 3×2.
3. **A-42 + A-40 · optimismo sistemático.** `cycleMode` siempre `'automatico'` (mejor caso de la Tabla 3.13)
...
4. **A-29 · f_machining por heurística.** Sustituimos el promedio ponderado por mezcla de procesos por
   `complexity > 2.5 ? 4 : …`, acoplando dos análisis que el libro mantiene separados. Multiplica el
   driver más grande del molde.
5. **A-31 · acabado por zonas sin cablear.** La API lo soporta y el test lo usa; el orquestador manda una
   sola zona. El dato existe y no llega al juez.
6. **A-53 · vetos declarados y no ejercidos.** `spec.vetos` está en el tipo, lo audita el contrato, y el
...
- `dfm-mesh.ts::dfmFromMesh` — mide sobre malla lo que el libro declara a mano, incluye `projectedAreaMm2`
  real (descuenta ventanas: el descuento que Kazmer hace mentalmente en A-43) y detecta cavidades cerradas
  no moldeables.
...
```
A-01 suficiencia del intake
  → A-02 restricción-vs-salida
      → A-03 cavidades mínimas por throughput   (necesita A-42, lazo corto)
  → A-04 tolerancias  ‖  A-05 regulatorio+estética  ‖  A-07 fichas de material
  → [FASE 1: DFM]
  → [FASE 2: costo del molde]
```

## Fase 1 — DFM (casi todo en paralelo, colgando de A-12)

```
A-12 uniformidad de pared  (RAÍZ del DFM: "todo lo demás hereda de aquí")
  → A-13 espesor nominal vs delgado+ribs
  → A-14 rib  ‖  A-15 boss  ‖  A-16 esquinas
                                  → A-17 radio de catálogo de herramienta
A-19 acabado ──┬→ A-18 draft requerido          (+1°/20 µm)
               ├→ pre-acabado obligatorio B/C
               └→ A-31 costo de acabado         (los TRES efectos de una sola decisión)
A-18 → A-20 undercuts (draft < 0)
        → A-21 eliminar vs conservar
A-04, A-05 ──→ A-10 checklist DFM 9 puntos ←── A-12, A-14, A-15, A-16, A-18, A-20
A-11 checklist DFA (independiente, mira el ensamble)
A-10, A-11, A-13, A-21 → A-22 propuesta de rediseño al cliente
```

## Fase 2 — Costo del molde (cadena casi lineal)

```
A-01 (bbox) → A-23 dimensiones de inserto
A-04 + A-07 → A-24 acero del inserto

A-23 + A-24 → A-25 material del inserto ────────────┐
A-23 + A-24 → A-26 t_vol  ‖  A-27 t_area            │
A-01 + A-12 → A-28 f_complexity                     │
[Fase 1] ────→ A-29 f_machining (mezcla de procesos)│
A-26,A-27,A-28,A-29 → A-30 costo de maquinado ──────┤
A-19 + A-05 ────────→ A-31 acabado por zonas ───────┤
                                                     ▼
                                            A-32 C_cavity (Ec 3.4)
                                                     │
                                            A-33 descuento ×n (Ec 3.3)
A-23 + n_cav → A-34 dims del molde → A-35 masa y costo de base (Ec 3.15)
A-20/A-21 + tecnologías → A-36 customización (Ec 3.18) ← A-33, A-35
                                                     ▼
                                    A-37 COSTO TOTAL DEL MOLDE (Ec 3.2)
```

## Fase 3 — Costo por pieza

```
A-24 + A-07 → A-38 f_maintenance
A-37 + A-38 → A-39 molde/pieza (Ec 3.20) ──┐
A-40 f_feed_waste → A-41 material/pieza ───┤
A-12 → A-42 t_cycle (Ec 3.23, h²) ─┐        │
A-01 → A-43 clamp (Ec 3.25) → A-44 R_maquina (Ec 3.24) │
A-42 + A-44 → A-45 proceso/pieza (Ec 3.22) ┤
A-01/A-04 → A-46 yield (Tabla 3.14) ───────┤
                                            ▼
                            A-47 COSTO POR PIEZA (Ec 3.19)
```

## Fase 4 — Lectura y cierre

```
A-37 + A-47 → A-48 costo total por escenario (Ec 3.1)
A-48 (×2)   → A-49 break-even
                → A-50 banda de sensibilidad
                → A-52 payback
A-02 + [toda la cadena] → A-51 barrido de escenarios (Tabla 3.1)
A-49 + A-52 → A-53 vetos no económicos → arquitectura FINAL
A-47        → A-54 veredicto de sobrediseño
A-37        → A-55 sanity check geográfico
A-37 + A-47 → A-57 costo vs precio + términos → A-58 calendario de desembolsos
A-34        → A-59 inserto ↔ base discreta
[todo]      → A-56 convergencia de la especificación → ENTREGA al cap. 4
```

## LOS RETORNOS (lo que hace que esto sea un grafo y no un wizard)

```
R1 ⇒ A-54 → A-51            SOBREDISEÑO. Si el molde amortizado domina el costo por pieza,
...
R2 ⇒ A-56 → A-51            CONVERGENCIA. "multiple cost estimates… until an effective mold
                            specification is established" (§3.5). El bucle exterior del capítulo.
                            PARCIAL: barremos una vez, no iteramos.

R3 ⇒ A-06 → A-01/A-12       GATE "Project OK?" (§1.5, Fig 1.9). Si el estimado rebasa la meta del
                            cliente, se regresa a "Review part design and specifications".
                            FALTA (no hay metas capturadas).

R4 ⇒ A-22 → A-12…A-21       REDISEÑO DE LA PIEZA. La propuesta al cliente reabre el DFM completo,
                            y con él toda la Fase 2 (A-28 f_complexity cambia). §2.2.4, §3.1.
                            FALTA.

R5 ⇒ A-21 → A-20 → A-36     UNDERCUT. Conservar o eliminar cambia el mecanismo, el coeficiente de
                            costo y a veces la dirección de apertura (que a su vez redefine QUÉ es
                            undercut: lazo A-20 ↔ cap. 4). §2.3.7. PARCIAL.

R6 ⇒ A-19 → A-18            ACABADO → DRAFT. Cambiar el acabado cambia el draft requerido
                            (+1°/20 µm) y por lo tanto puede crear undercuts nuevos (A-20).
                            El retorno más olvidado del cap. 2. TENEMOS la fórmula, no el disparo.

R7 ⇒ A-59 → A-23            BASE DISCRETA. "iteration between the inserts' sizing and mold base
                            selection is normal" (cap. 4 intro). Re-dimensionar el inserto
                            re-dispara TODA la Fase 2. PARCIAL: aterrizamos, no iteramos.

R8 ⇒ A-53 → A-49            VETO. Un factor no económico tumba al ganador del break-even y obliga
                            a elegir el segundo lugar. §3.2.2. DECLARADO, NO EJERCIDO.

R9 ⇒ A-03 → A-51            THROUGHPUT. Si ni la cavitación máxima alcanza el volumen anual,
                            hay que replantear (más moldes, o ciclo más corto → A-13 → A-12).
                            TENEMOS (bandera `throughputForzado`).

R10 ⇒ A-13 → A-42 → A-45    ESPESOR. Engrosar para ganar rigidez multiplica el ciclo por h²
                            y con él el costo de proceso; el "arreglo" de rigidez se paga
                            en la Fase 3. TENEMOS las piezas, FALTA el comparador.

R11 ⇒ A-08 (transversal)    CONFLICTO ENTRE SUBSISTEMAS. "the placement of ejector(s) may require
...
- **Fase 0:** A-04 ‖ A-05 ‖ A-07 (tres worksheets independientes).
- **Fase 1:** A-14 ‖ A-15 ‖ A-16 ‖ A-18 ‖ A-20 (cinco verificadores geométricos, todos cuelgan de A-12).
- **Fase 2:** A-25 ‖ (A-26 ‖ A-27) ‖ A-28 ‖ A-31 convergen en A-32; y A-34→A-35 corre en paralelo a
  toda la rama de insertos.
- **Fase 3:** A-39 ‖ A-41 ‖ A-45 son independientes entre sí; solo A-47 los junta.
- **Fase 4:** las 15 variantes de A-51 son **vergonzosamente paralelas** — cada una es una corrida
  completa e independiente de A-23…A-48.
