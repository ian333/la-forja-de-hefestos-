/**
 * LimonesEscena21 — "Tu vida ya cambió" (Cap 5 · hook)
 *
 * México focus. Tres ejemplos cotidianos: Tsuru, Tepito, Mercado Libre.
 * Cada uno cierra con "eso es Akerlof".
 *
 * Duración: ~31s.
 */
...
function CameraDirector({ timeRef }: { timeRef: React.MutableRefObject<number> }) {
  ...
  useFrame(() => {
    ...
    camera.position.set(0.25 * Math.sin(t * 0.10), 0.6, 9 - 0.6 * clamp(t / 28, 0, 1));
...
function SceneContent({ audioRef, isPlaying }: {
  ...
  const mexRef = useRef<SkyTextHandle | null>(null);
  const ex1Ref = useRef<SkyTextHandle | null>(null);
  const ex1TagRef = useRef<SkyTextHandle | null>(null);
  const ex2Ref = useRef<SkyTextHandle | null>(null);
  const ex2TagRef = useRef<SkyTextHandle | null>(null);
  const ex3Ref = useRef<SkyTextHandle | null>(null);
  const ex3TagRef = useRef<SkyTextHandle | null>(null);
  ...
  useFrame((_, dt) => {
    ...
    const fades: Array<[React.RefObject<SkyTextHandle | null>, number, number, number?]> = [
      [mexRef, 1, 2.5, 1.0],
      [ex1Ref, 4, 2, 0.9],
      [ex1TagRef, 7.5, 1.5, 0.9],
      [ex2Ref, 10, 2, 0.9],
      [ex2TagRef, 13.5, 1.5, 0.9],
      [ex3Ref, 16, 2, 0.9],
      [ex3TagRef, 22, 3, 1.0],
    ];
  ...
  return (
    <>
      ...
      <SkyText ref={mexRef} text="aquí · MÉXICO" position={[0, 3.8, -3]} width={6.5} height={0.85} color="#FFE5A0" fontWeight={700} />

      {/* Ejemplo 1: Tsuru → 20% */}
      <SkyText ref={ex1Ref} text="Tsuru sale del lote" position={[0, 2.0, 0]} width={6.0} height={0.55} color="#FFE5A0" fontWeight={500} />
      <SkyText ref={ex1TagRef} text="pierde 20% · Akerlof" position={[0, 1.1, 0]} width={6.0} height={0.45} color="#FFB81C" fontWeight={600} />

      {/* Ejemplo 2: Tepito */}
      <SkyText ref={ex2Ref} text="refacciones · Tepito" position={[0, -0.2, 0]} width={6.0} height={0.55} color="#FFE5A0" fontWeight={500} />
      <SkyText ref={ex2TagRef} text="¿originales? ¿genéricas? · Akerlof" position={[0, -1.1, 0]} width={8.5} height={0.45} color="#FFB81C" fontWeight={600} />

      {/* Ejemplo 3: Mercado Libre */}
      <SkyText ref={ex3Ref} text="Mercado Libre · 100 vendedores" position={[0, -2.4, 0]} width={8.5} height={0.55} color="#FFE5A0" fontWeight={500} />
      <SkyText ref={ex3TagRef} text="todos prometen lo mismo · Akerlof" position={[0, -3.3, 0]} width={9.0} height={0.50} color="#FFB81C" fontWeight={600} />
...
export default function LimonesEscena21({ forceAspect = 'auto' }: Props) {
      ...
      {!hasStarted && !isScreenshotMode && (
        <button onClick={handlePlay} className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm cursor-pointer group" style={{ zIndex: 50 }}>
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-[11px] uppercase tracking-[0.3em] text-[#FFE5A0]/70 font-mono">Cap 5 · Escena 21 · {audioMeta ? audioMeta.duration.toFixed(0) + 's' : '~31s'}</div>
            <div className="text-[10px] text-[#FFE5A0]/40 font-mono">tu vida ya cambió</div>
          </div>
        </button>
      )}
    </div>
