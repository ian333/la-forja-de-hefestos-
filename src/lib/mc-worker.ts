/**
 * ⚒️ La Forja — Marching Cubes Web Worker
 * =========================================
 * Runs CPU-side SDF evaluation + Marching Cubes in a background thread.
 * Communicates via postMessage with transferable ArrayBuffers for zero-copy.
 *
 * Protocol:
...
// We inline it to avoid import issues in the worker context.
...
type SdfNode = SdfPrimitive | SdfOperation;
...
const TRI_TABLE: number[][] = new Array(256).fill(null).map(() => [-1]);
...
function marchingCubes(
  ...
  const idx = (x: number, y: number, z: number) => x + size * (y + size * z);
  ...
  // Compute per-vertex normals via SDF gradient (for smooth shading + engineer precision)
  const normals = new Float32Array(positions.length);
  const eps = (step[0] + step[1] + step[2]) / 3 * 0.5;
  for (let i = 0; i < positions.length; i += 3) {
    const px: V3 = [positions[i], positions[i+1], positions[i+2]];
    const nx = evalNode(scene, [px[0]+eps, px[1], px[2]]) - evalNode(scene, [px[0]-eps, px[1], px[2]]);
    const ny = evalNode(scene, [px[0], px[1]+eps, px[2]]) - evalNode(scene, [px[0], px[1]-eps, px[2]]);
    const nz = evalNode(scene, [px[0], px[1], px[2]+eps]) - evalNode(scene, [px[0], px[1], px[2]-eps]);
...
function computeBounds(scene: SdfNode): [V3, V3] {
  ...
  function walk(node: SdfNode) {
    if (node.kind === 'primitive') {
      ...
      let r = 1;
      switch (pr.type) {
        case 'sphere': r = (p.radius ?? 1); break;
        case 'box': r = Math.max(p.sizeX ?? 1, p.sizeY ?? 1, p.sizeZ ?? 1) * 0.5; break;
        case 'cylinder': r = Math.max(p.radius ?? 0.5, (p.height ?? 1) * 0.5); break;
        case 'torus': r = (p.majorRadius ?? 1) + (p.minorRadius ?? 0.25); break;
        case 'cone': r = Math.max(p.radius ?? 0.5, p.height ?? 1); break;
        case 'capsule': {
          const ax = p.ax ?? 0, ay = p.ay ?? 0, az = p.az ?? 0;
          const bx = p.bx ?? 0, by = p.by ?? 1, bz = p.bz ?? 0;
          const cr = p.radius ?? 0.02;
          minX = Math.min(minX, ax - cr, bx - cr);
          minY = Math.min(minY, ay - cr, by - cr);
          minZ = Math.min(minZ, az - cr, bz - cr);
          maxX = Math.max(maxX, ax + cr, bx + cr);
          maxY = Math.max(maxY, ay + cr, by + cr);
          maxZ = Math.max(maxZ, az + cr, bz + cr);
          return;
        }
      }
      minX = Math.min(minX, pr.position[0] - r);
      minY = Math.min(minY, pr.position[1] - r);
      minZ = Math.min(minZ, pr.position[2] - r);
      maxX = Math.max(maxX, pr.position[0] + r);
      maxY = Math.max(maxY, pr.position[1] + r);
      maxZ = Math.max(maxZ, pr.position[2] + r);
    } else {
...
self.onmessage = (e: MessageEvent) => {
  const { type, scene, resolution, bounds } = e.data;

  if (type === 'mesh') {
    const actualBounds = bounds || computeBounds(scene);
    const result = marchingCubes(scene, resolution || 128, actualBounds);
