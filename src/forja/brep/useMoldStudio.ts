...
import { measureFlowLength } from '../mold/flowlen';
import { convergeVelocityCross, ABS_CROSS, PP_CROSS, type MeltMaterial } from '../mold/filling';
import { FEED_MATERIALS } from '../mold/feed';
import { PLASTICOS_A } from '../mold/cooling-design';
// SPRINT 2 (superticket moldes): la REOLOGÍA sigue al intake §2.1.5 — SOLO donde el
// Apéndice A tiene datos (ABS Cycolac MG47 p.392 · PP Dow Inspire 702 p.393). Para lo
// demás se usa ABS y se DECLARA en el panel: número declarado > número adivinado.
// ABS devuelve los MISMOS objetos de siempre (bit-igual para el cubo).
function reologiaDe(material?: string): { cross: typeof ABS_CROSS; melt: MeltMaterial; kappa: number; tWall: number; nota: string } {
  if (material === 'PP') {
    const f = FEED_MATERIALS.PP, kappa = PLASTICOS_A.PP?.kWmC ?? 0.19;
    return { cross: PP_CROSS, melt: { k: f.k, n: f.n, kappa, tMelt: f.tMelt, tWall: f.tCool }, kappa, tWall: f.tCool, nota: 'PP Dow Inspire 702 — Cross-WLF y power-law del Apéndice A (p. 393)' };
  }
  const base = { cross: ABS_CROSS, melt: ABS_MG47, kappa: 0.19, tWall: 60 };
  if (!material || material === 'ABS') return { ...base, nota: 'ABS Cycolac MG47 — Apéndice A (p. 392)' };
  return { ...base, nota: `⚠ sin Cross-WLF del Apéndice A para ${material}: se usa la reología de ABS (DECLARADO, no adivinado)` };
}
...
export function useMoldStudio({ oc, setCollapsed, setDocName, arbol, arbolRev = 0 }: {
  ...
  // 0..1 — el instante del frente. VA CON REF además del estado: la API de
  // `__forgeBrep` se arma una vez y su clausura congelaba `tFill` en 1, así que
  // `llenadoStats()` reportaba 100 % de llenado en TODOS los frames (lo cazó el
  ...
  const [ciclo, setCiclo] = useState<{ estacion: number; e1: Estacion1Dado;
    ...
    e7?: Estacion7Dado;
    /** ESTACIÓN 8 · ENFRIAMIENTO (cap 9): el rey del ciclo — agua + el bebedero que manda */
    e8?: Estacion8Dado;
    /** ESTACIÓN 9 · CONTRACCIÓN (cap 10): el acero ESCALADO — el retorno de la E3 pagado */
    e9?: Estacion9Dado;
    /** ESTACIÓN 10 · EXPULSIÓN (cap 11): los pines — los ciclos ejercidos */
    e10?: Estacion10Dado;
    /** ESTACIÓN 11 · ESTRUCTURA (cap 12): el acero que quedó, rindiendo cuentas */
    e11?: Estacion11Dado;
    /** ESTACIÓN 12 · EL ACTA (§13.10): el cubo, firmado */
    e12?: ActaDado;
    ...
    espiralExacta?: { positions: Float32Array; normals: Float32Array; indices: Uint32Array; sV: Float32Array; fillV: Float32Array; sFront: Float32Array;
      /** N2: el reloj del CONGELAMIENTO — la escena tiñe cada arco a acero-frío por su edad */
  ...
  // SPRINT 2 (superticket moldes) — EL INTAKE §2.1.5. Lo que el árbol NO sabe de la
  // pieza lo DECLARA el usuario: pared (un STEP no trae cascarón), material (el
  // puente lo fijaba en ABS) y volumen anual. Sin formulario aparte: tres campos en
  // la E1 que re-siembran el ciclo. `intakeRev` obliga a re-sembrar aunque el sólido
  // sea el mismo (el guard de "mismo shape" lo bloqueaba).
  const [intake, setIntakeState] = useState<{ wallMm?: number; material?: string; annualVolume?: number }>({});
  const [intakeRev, setIntakeRev] = useState(0);
  const intakeRef = useRef(intake);
  const setIntake = useCallback((patch: { wallMm?: number; material?: string; annualVolume?: number }) => {
    const next = { ...intakeRef.current, ...patch };
    intakeRef.current = next; setIntakeState(next); setIntakeRev((v) => v + 1);
  }, []);
  const intakeAplicadoRef = useRef(-1);
  useEffect(() => {
    ...
    if (esDemo) return;
    if (ciclo?.pieza && ciclo.piezaShape === a.shape && intakeAplicadoRef.current === intakeRev) return;   // mismo sólido y mismo intake: nada que hacer
    try {
      const ik = intakeRef.current;
      const pieza = piezaDesdeArbol(oc, a.shape, {
        nombre: a.nombre || 'PIEZA DEL ÁRBOL', wallMm: ik.wallMm ?? a.wallMm, draftDeg: a.draftDeg,
        filletMm: a.filletMm, round: a.round, material: ik.material ?? 'ABS',   // intake §2.1.5 (sprint 2): declarado > adivinado
        annualVolume: ik.annualVolume,
      });
      intakeAplicadoRef.current = intakeRev;
  ...
  }, [oc, arbolRev, intakeRev]);  // eslint-disable-line react-hooks/exhaustive-deps -- ciclo/moldParts se leen como guardas, no disparan
  ...
  // ── LA COLA DE PUERCO (orden la-cola-de-puerco): la espiral de flujo de la
  // patente US11976138, contra los 552 mm MEDIDOS de US11230635 (ABS @238 °C).
  // Primera vez que el solver se mide contra números ajenos, EN VIVO en el CAD.
  const loadEspiral = useCallback(() => {
    if (!oc) return;
    try {
      const e1 = estacion1Dado();
      const esp = campoEspiral({ TmeltC: 238 });
      const campo = esp.campo;
      const fanE = resolverLlenadoFAN(campo, {
  ...
  // ── N2 TÉRMICO (orden 2026-08-17-n2-termico): la cola de puerco MUERE CONGELADA.
  // Las 3 isotermas de la patente corren AQUÍ con el MISMO protocolo del gate y el
  // panel imprime la tabla con veredicto. TODO se computa ANTES de armar la escena
  // (clic ~2 min, avisado en el título): si se computara después, el video quedaría
  // congelado a media captura por el hilo bloqueado.
  const loadEspiralN2 = useCallback(() => {
    if (!oc) return;
    try {
      const e1 = estacion1Dado();
      const c238 = espiralN2Corrida(238);
      const c249 = espiralN2Corrida(249);
      const c260 = espiralN2Corrida(260);
      const esp = c238.esp, fanE = c238.r, campo = esp.campo;
      const ocu = esp.ocupacionSdf;
      const nCav = campo.cavity.reduce((a: number, b: number) => a + b, 0);
      const posE = new Float32Array(nCav * 3); const fvE = new Float32Array(nCav);
      const esP = new Uint8Array(nCav).fill(1);
      ...
      for (let k = 0; k < campo.nz; k++) for (let j = 0; j < campo.ny; j++) for (let i = 0; i < campo.nx; i++) {
        const id = campo.idx(i, j, k);
        if (!campo.cavity[id]) continue;
        posE[q * 3] = campo.x0 + (i + 0.5) * campo.cellMm; posE[q * 3 + 1] = campo.y0 + (j + 0.5) * campo.cellMm; posE[q * 3 + 2] = campo.z0 + (k + 0.5) * campo.cellMm;
        fvE[q] = fanE.frente[id]; q++;
      }
      const fqE = Float32Array.from([...fvE].filter((x) => x >= 0)).sort();
      const malla = espiralMalla(esp, 2);
      ...
      setCiclo({
        ...
        frenteVert: fvE, frenteQ: fqE, voxPos: posE, cellMm: campo.cellMm, esPieza: esP, ocupacion: ocu,
        espiralExacta: {
          positions: malla.positions, normals: malla.normals, indices: malla.indices, sV: malla.sV, fillV, sFront,
          n2: { tFillS: fanE.tFillS, tcS: c238.tcCanalS },
        },
        e5fan: { pMaxMPa: fanE.pMaxMPa, tFillS: fanE.tFillS, shortShot: fanE.shortShot, incompleto: fanE.incompleto, conservacionMaxRel: fanE.conservacionMaxRel, etaEffPaS: fanE.etaEffPaS, QmmS: fanE.QmmS, nNodos: fanE.nNodos },
      });
      const sg = (x: number | null) => (x != null && x > 0 ? `+${x}` : `${x}`);
      setDocName(`N2 TÉRMICO · la cola de puerco MUERE CONGELADA — 238°: ${c238.Lmm} mm vs 552 (${sg(c238.dPct)} %) · Q muere a ${(100 * fanE.fase2.qFinalFrac).toFixed(1)} % de Q₀`);
      setCollapsed((c) => ({ ...c, features: false }));
      const fila = (cc: typeof c238) => `${cc.TC} °C: ${cc.Lmm} mm vs ${cc.Lexp} patente (${sg(cc.dPct)} %) ${cc.cumple ? '✓' : '✗'} banda ±20 (sesgo +13..16 declarado: grado + intensificación nominal)`;
      cursoSet(1, [
        'N2 TÉRMICO — el freno que mata al creep: piel erf × Cross-WLF del corazón × power-law',
        fila(c238), fila(c249), fila(c260),
        `PENDIENTE ${((c260.Lmm - c238.Lmm) / 22).toFixed(2)} vs 8.09 mm/°C medida · COCIENTES 249/238 ${(c249.Lmm / c238.Lmm).toFixed(3)} vs 1.150 · 260/238 ${(c260.Lmm / c238.Lmm).toFixed(3)} vs 1.322 (grado y presión SE CANCELAN)`,
        `t_congela del canal ${c238.tcCanalS} s (centro, h=3.175) · ayer isotermo: 799.8 mm DESBOCADO (gate) · hoy el creep muere solo: Q → ${(100 * fanE.fase2.qFinalFrac).toFixed(1)} %`,
      ], [
        cursoPart(acero.placa, 'placa-espiral', 'PLACA TALLADA — el canal 12.7×3.175 hundido en el acero (occCut)', '#8fa8c8', 0.30, 0.35),
        cursoPart(acero.tapa, 'tapa-espiral', 'TAPA plana — cierra el canal en la partición', '#8fa8c8', 0.10, 0.15),
      ]);
    } catch (e) { console.warn('ESPIRAL_N2_ERR', e); }
  }, [oc, cursoSet, cursoPart, setDocName, setCollapsed]);
  ...
  // OJO: `onClick={fn}` pasa el EVENTO de React como 1er argumento. Con un flag
  ...
  const cicloEstacion3 = useCallback((maloArg: unknown = false) => {
    ...
    try {
      ...
      const marco3 = pieza?.local ? {
        ...
        wall: pieza.pieza.wallMm, piso: pieza.pieza.wallMm, draftDeg: pieza.pieza.part.draftDeg ?? 0,
      } : undefined;
  ...
  const cicloEstacion4 = useCallback(() => {
    ...
    try {
      const pieza = moldParts.find((p) => p.role === 'pieza');
      if (!pieza) return;
      // SPRINT 1 (superticket moldes) — E4 POR PIEZA. El cubo conserva su predicado
      // ANALÍTICO (dentroDadoLocal, bit-igual: la caja del campo y la semilla salen de
      // DADO_PIEZA.local y reproducen los literales 40/20/39.5 de siempre). La pieza del
      // árbol usa el PREDICADO DE SU MALLA (paridad de rayo, flowlen.ts): la malla del
      // rol 'pieza' ya vive VOLTEADA y COLOCADA en la base — el mismo marco del campo.
      const piezaSpec = ciclo.pieza;
      const lo = piezaSpec?.local ?? DADO_PIEZA.local!;
      const wallMm = piezaSpec ? piezaSpec.pieza.wallMm : 2;
      ...
      const col = colocacionEnLaBase(ciclo.e2.pkg, piezaSpec?.local);
      const dentroBase = piezaSpec
        ? predicadoDeMalla(pieza.positions, pieza.indices)
        : (X: number, Y: number, Z: number) => { const q = col.aLocal(X, Y, Z); return dentroDadoLocal(q[0], q[1], q[2]); };
      // el SPRUE GATE: centro del piso (base cerrada), a media pared — DENTRO del plástico
      const gSem = col.aGlobal(lo.cxMm, lo.cyMm, piezaSpec ? wallMm / 2 : 1);
      const rh4 = reologiaDe(piezaSpec?.material);
      const campo = measureFlowLength({
        x0: col.tx + (lo.cxMm - lo.semiXmm) - 2, y0: col.ty - (lo.cyMm + lo.semiYmm) - 2, z0: col.tz - lo.alturaMm - 2,
        x1: col.tx + (lo.cxMm + lo.semiXmm) + 2, y1: col.ty - (lo.cyMm - lo.semiYmm) + 2, z1: col.tz + 2, cellMm: 1.0,
        ...
        inCavity: dentroBase, gateMm: { x: gSem[0], y: gSem[1], z: gSem[2] }, wallMm, meltN: rh4.melt.n,
      });
      const cw = convergeVelocityCross(rh4.cross, rh4.kappa, rh4.tWall, wallMm / 1000);
      const n1 = llenadoNivel1(campo, { vMs: cw.vMs, muPaS: cw.muFinalPaS, wallMm, material: rh4.melt });
      // MALLA FINA para el frente: la pieza teselada del CAD trae ~48 vértices (caras
      // planas = pocos triángulos). Un fluido pintado sobre 48 puntos NO se ve avanzar
      // — el check de píxeles del video lo cazó (0 % de cambio en 170 frames). Aquí se
      // re-tesela el MISMO sólido fino, solo para pintar.
      // el fundido se dibuja como VÓXELES (el campo ES volumétrico): centro + frente
      // de cada celda de cavidad. Pintar vértices fracasó — el dado es todo caras
      // planas y da ~7 vértices; el check de píxeles del video lo cazó.
      const nCav = campo.cavity.reduce((a: number, b: number) => a + b, 0);
      const pos = new Float32Array(nCav * 3); const fv = new Float32Array(nCav);
      let q = 0;
      for (let k2 = 0; k2 < campo.nz; k2++) for (let j2 = 0; j2 < campo.ny; j2++) for (let i2 = 0; i2 < campo.nx; i2++) {
        const id = campo.idx(i2, j2, k2);
        if (!campo.cavity[id]) continue;
        pos[q * 3] = campo.x0 + (i2 + 0.5) * campo.cellMm;
        pos[q * 3 + 1] = campo.y0 + (j2 + 0.5) * campo.cellMm;
        pos[q * 3 + 2] = campo.z0 + (k2 + 0.5) * campo.cellMm;
        fv[q] = n1.frente[id];
        q++;
      }
      const voxPos = pos;
      // EL RELOJ DEL VIDEO ES EL VOLUMEN, NO LA PRESIÓN. La inyectora empuja a CAUDAL
      // CONSTANTE (el husillo avanza a velocidad fija), así que el volumen llenado crece
      // LINEAL en el tiempo. La resistencia da el ORDEN de llenado (Eq 5.22) pero NO el
      // reloj: usarla como eje comprimía el arranque y estiraba el final (el juicio del
      // video lo cazó: 25 % del tiempo → 4 % de volumen). Los CUANTILES del frente
      // convierten orden en tiempo: fracción de volumen u → umbral de resistencia.
      const frenteQ = Float32Array.from([...fv].filter((x) => x >= 0)).sort();
      const sf = { maxFlowLenMm: campo.maxFlowLenMm, flowLenMm: campo.flowLenMm };
      const e4 = estacion4Dado(ciclo.e2.pkg, campo.maxFlowLenMm, wallMm);
      e4.reologia = rh4.nota;
      // la ÚLTIMA zona: el vértice más lejano por el camino del fundido
      let iMax = 0; for (let i = 1; i < sf.flowLenMm.length; i++) if (sf.flowLenMm[i] > sf.flowLenMm[iMax]) iMax = i;
      e4.ultimaZona = {
  ...
  // pared de 2 mm, sin runner ni compuerta — y los radios estaban HARDCODEADOS aquí
  // (`makeCone(oc, 4.75, 2.5, 60)`) mientras `designSprueFeed` ya los calculaba y el
  ...
  const cicloEstacion5 = useCallback(() => {
    ...
    try {
      ...
      const piezaSpec5 = ciclo.pieza;
      const lo5 = piezaSpec5?.local ?? DADO_PIEZA.local!;
      const wallMm = piezaSpec5 ? piezaSpec5.pieza.wallMm : 2;
      ...
      const col = colocacionEnLaBase(ciclo.e2.pkg, piezaSpec5?.local);
      const hx0 = col.tx + (lo5.cxMm - lo5.semiXmm), hx1 = col.tx + (lo5.cxMm + lo5.semiXmm);
      const hy0 = col.ty - (lo5.cyMm + lo5.semiYmm), hy1 = col.ty - (lo5.cyMm - lo5.semiYmm);
      const rh5 = reologiaDe(piezaSpec5?.material);
      ...
      const piezaMesh5 = moldParts.find((p) => p.role === 'pieza');
      const dentroPiezaBase = piezaSpec5 && piezaMesh5
        ? predicadoDeMalla(piezaMesh5.positions, piezaMesh5.indices)
        : (X: number, Y: number, Z: number) => { const q = col.aLocal(X, Y, Z); return dentroDadoLocal(q[0], q[1], q[2]); };
      const dentroTuberia = (x: number, y: number, z: number) => dentroPiezaBase(x, y, z) || denCol(x, y, z);
      const campoJ = measureFlowLength({
        x0: Math.min(d.ejeX - d.rBaseMm, hx0) - 2, y0: Math.min(d.ejeY - d.rBaseMm, hy0) - 2,
        z0: col.tz - (lo5.alturaMm + 1) - 2,
        x1: Math.max(d.ejeX + d.rBaseMm, hx1) + 2, y1: Math.max(d.ejeY + d.rBaseMm, hy1) + 2,
  ...
  // pidió ian: el reloj del video encoge la pieza ×20 (rotulado SIEMPRE) — el
  // número real manda en filas y gates; la exageración es solo del ojo.
  const cicloEstacion6 = useCallback(() => {
    if (!ciclo || ciclo.estacion !== 5 || !ciclo.e5datums || !ciclo.e2) return;
    try {
      const e6 = estacion6Dado(ciclo.e2.pkg, ciclo.e5datums, {
        partVolCc: ciclo.e5tuberia?.volPiezaCc ?? 14.14,
      });
      setTFill(0); tFillRef.current = 0;                 // el reloj ahora es EMPAQUE/enfriamiento
      setCiclo({ ...ciclo, estacion: 6, e6 });
      setDocName(`EL DADO · estación 6 — EMPAQUE: ⚠ CONTRACCIÓN EXAGERADA ×${e6.contraccion.exag} (real: ${e6.contraccion.linealPct} %)`);
      setCollapsed((c: any) => ({ ...c, features: false }));
      cursoSet(6, [
        'CICLO DEL DADO · estación 6 — EMPAQUE (cap 7)',
        `⚠ LA ESCENA EXAGERA LA CONTRACCIÓN ×${e6.contraccion.exag} — real: ${e6.contraccion.linealPct} % lineal (Tait a ${e6.pPackMPa} MPa)`,
        `gate REAL (sprue ⌀${e6.gate.diaOrigMm}): freeze ${e6.gate.freezeOrigS}s ${e6.gate.freezeCortoOrig ? '<' : '≥'} t_pack ${e6.gate.tPackNeededS}s — el anuncio de la E4 ${e6.gate.freezeCortoOrig ? 'sigue abierto' : 'SE CIERRA: la Fig 7.2 lo resolvió'}`,
        `masa del disparo ${e6.masa.disparoG} g · moldScale ${e6.contraccion.moldScale} → E9`,
      ], moldParts);
  ...
  // ── ESTACIÓN 8: ENFRIAMIENTO (cap 9) — EL REY DEL CICLO. El motor
  // (coolingDesign, 7 pasos §9.2) ya existía; aquí se ata al dado, se tallan
  // LAS LÍNEAS DE AGUA reales (⌀D a profundidad H y pitch W del diseño, ambos
  // lados) y se cobra el hallazgo: el BEBEDERO manda el ciclo (×8.2).
  // Sin exageración: ⌀4.76 y H 19 mm SE VEN a escala real.
  const cicloEstacion8 = useCallback(() => {
    if (!oc || !ciclo || ciclo.estacion !== 7 || !ciclo.e5datums || !ciclo.e2) return;
    try {
      const d = ciclo.e5datums;
      const e8 = estacion8Dado(ciclo.e2.pkg, d, {
        partVolCc: ciclo.e5tuberia?.volPiezaCc ?? 14.14,
      ...
      setDocName(`EL DADO · estación 8 — ENFRIAMIENTO: el BEBEDERO manda el ciclo (${e8.ciclo.tcSprueS}s vs ${e8.ciclo.tcPiezaS}s de la pieza = ×${e8.ciclo.factor})`);
      setCollapsed((c: any) => ({ ...c, features: false }));
      cursoSet(8, [
  ...
  // ── ESTACIÓN 9: CONTRACCIÓN (cap 10) — el acero SE TALLA ESCALADO. El gancho
  // de splitMold (escala 1.0, declarado en la E3) se jala con la mezcla
  // steel-safe §10.2.2(A): cavidad s bajo / macho s alto. Las cotas 3D llevan
  // la doble cifra nominal→tallado MEDIDA del B-Rep (regla de ian desde la E3).
  const cicloEstacion9 = useCallback(() => {
    if (!oc || !ciclo || ciclo.estacion !== 8 || !ciclo.e2 || !ciclo.e5datums) return;
    try {
      const e9 = estacion9Dado(ciclo.e2.pkg);
      const a1 = construirAceroE3(oc, ciclo.e2.pkg);                  // baseline escala 1.0
      const a9 = construirAceroE3(oc, ciclo.e2.pkg, false, e9.escala); // el TALLADO steel-safe
      // MEDIR del sólido: macho (escala core) por bbox; cavidad por VOLUMEN del
      // hueco (iso-escala ⇒ X equivalente = 40·(hueco9/hueco1)^⅓)
      const bb1 = shapeBBox(oc, a1.r.macho), bb9 = shapeBBox(oc, a9.r.macho);
      const machoX1 = bb1.max[0] - bb1.min[0], machoX9 = bb9.max[0] - bb9.min[0];
      const blockVol = 120 * 120 * 60;
      const hueco1 = blockVol - OCC.volume(oc, a1.r.cavityPlate);
      const hueco9 = blockVol - OCC.volume(oc, a9.r.cavityPlate);
      const cavXeq = 40 * Math.cbrt(hueco9 / hueco1);
      const d = ciclo.e5datums;
      const cx = d.ejeX, cy = d.ejeY, zPart = d.zPartMm;
      // partes RE-TALLADAS en los MISMOS roles (colores/opacidad heredados)
      const shapeDe: Record<string, any> = { cavidad: a9.r.cavityPlate, nucleo: a9.r.macho, particion: a9.r.corePlate };
      const parts = moldParts.map((pt: any) => shapeDe[pt.role]
        ? cursoPart(shapeDe[pt.role], pt.role, `${pt.name.split(' · ESCALADO')[0]} · ESCALADO E9 (cav ×${e9.escala.cav.toFixed(4)} · macho ×${e9.escala.core.toFixed(4)})`, pt.color, 0.34, 0.4)
        : pt);
      // COTAS 3D con doble cifra (la tubería de la E3): declarado = nominal×(1+s)
      const mm = (x: number) => +x.toFixed(2);
      const cotas = [{
        role: 'nucleo', dims: [
          { id: 'e9-macho', label: `macho base ${mm(machoX1)} → tallado`, kind: 'linear', a: [cx - machoX9 / 2, cy - 26, zPart + 2], b: [cx + machoX9 / 2, cy - 26, zPart + 2], value: mm(machoX1 * e9.escala.core), measured: mm(machoX9), ok: Math.abs(machoX9 - machoX1 * e9.escala.core) < 0.05 },
          { id: 'e9-cav', label: 'cavidad X 40 → tallado (por volumen del hueco)', kind: 'linear', a: [cx - cavXeq / 2, cy + 26, zPart + 41], b: [cx + cavXeq / 2, cy + 26, zPart + 41], value: mm(40 * e9.escala.cav), measured: mm(cavXeq), ok: Math.abs(cavXeq - 40 * e9.escala.cav) < 0.08 },
          { id: 'e9-descalce', label: `descalce shutoff (reserva steel-safe, spotting)`, kind: 'linear', a: [cx + 20, cy, zPart], b: [cx + 20 + 8, cy, zPart], value: e9.descalceShutoffMm, measured: mm((machoX9 / machoX1 - cavXeq / 40) * 40 / 2 * 100) / 100, ok: true },
        ],
      }] as any;
      setTFill(1); tFillRef.current = 1;
      setCiclo({ ...ciclo, estacion: 9, e9, e3cotas: cotas });
      setCotasOn(true);
      setDocName(`EL DADO · estación 9 — CONTRACCIÓN: acero ESCALADO (cav ×${e9.escala.cav.toFixed(4)} · macho ×${e9.escala.core.toFixed(4)}) — el retorno de la E3 CERRADO`);
      setCollapsed((c: any) => ({ ...c, features: false }));
      cursoSet(9, [
        'CICLO DEL DADO · estación 9 — CONTRACCIÓN (cap 10): el acero escalado',
        `fuentes confrontadas: proveedor 0.5–0.8 % · Tait débil ${e9.brecha.taitDebilPct} % (brecha MOSTRADA) · Tait perilla ${e9.brecha.taitPerillaPct} % → convergen`,
        `steel-safe (A): cavidad ×${e9.escala.cav.toFixed(4)} · macho ×${e9.escala.core.toFixed(4)} · descalce ${e9.descalceShutoffMm} mm/lado (spotting) — MEDIDO: macho ${mm(machoX9)} vs ${mm(machoX1 * e9.escala.core)} esperado`,
        `techo del proceso: s=0 a ~${e9.pTechoMPa} MPa (la alarma §10.1.6 VIVA) · pandeo de la tapa si ΔT > ${e9.pandeo.dTcritC} °C (defensa: E8b)`,
      ], parts);
    } catch (e) { console.warn('E9_ERR', e); }
  }, [oc, ciclo, moldParts, cursoPart, cursoSet, setDocName, setCollapsed, setCotasOn]);
  // ── ESTACIÓN 10: EXPULSIÓN (cap 11) — los 8 pines ESCALONADOS reales (punta
  // ⌀1.588 guiada + cuerpo ⌀3.175), colocados por el ciclo de decisión: R34→R46
  // (pared), R52 (pandeo→escalonado) y A-239 al revés (ceden el carril del
  // baffle). "El molde no es un pipeline" — ian.
  const cicloEstacion10 = useCallback(() => {
    if (!oc || !ciclo || ciclo.estacion !== 9 || !ciclo.e2 || !ciclo.e5datums || !ciclo.e8) return;
    try {
      const d = ciclo.e5datums;
      const e10 = estacion10Dado(ciclo.e2.pkg, d, ciclo.e8.circuito);
      const zBoca = d.zPartMm;
      const punta = e10.pines.escalonado;
      const parts: any[] = [];
      for (let k = 0; k < e10.pines.posiciones.length; k++) {
        const pin = e10.pines.posiciones[k];
        const zHombro = zBoca - punta.puntaMm;
        const tip = OCC.makeCylinder(oc, punta.puntaDiaMm / 2, punta.puntaMm, { origin: [pin.x, pin.y, zHombro], dir: [0, 0, 1] });
        const body = OCC.makeCylinder(oc, punta.cuerpoDiaMm / 2, punta.libreCuerpoMm, { origin: [pin.x, pin.y, zHombro - punta.libreCuerpoMm], dir: [0, 0, 1] });
        parts.push(
          cursoPart(tip, `pin-punta-${k}`, `PIN ${k} (${pin.tipo}) · punta ⌀${punta.puntaDiaMm} × ${punta.puntaMm} GUIADA en su barreno — ${pin.tipo === 'esquina-knit' ? 'pin-VENT de la knit E7 (claro 0.13 ⇒ vent 0.065)' : 'medio-lado (corrido del carril del baffle: A-239)'}`, '#d8dde4', 0.95, 0.95),
          cursoPart(body, `pin-cuerpo-${k}`, `cuerpo ⌀${punta.cuerpoDiaMm} al hombro (+1 → catálogo, R52) — el pandeo se verifica AQUÍ (SF ${e10.pines.pandeoNuestro.sf})`, '#aab4c0', 0.9, 0.9),
        );
      }
      setTFill(1); tFillRef.current = 1;
      setCiclo({ ...ciclo, estacion: 10, e10 });
      setDocName(`EL DADO · estación 10 — EXPULSIÓN: 8 pines escalonados (⌀${punta.puntaDiaMm}/${punta.cuerpoDiaMm}) · F ${e10.fuerza.fEjectN} N · los ciclos EJERCIDOS (R34→R46 · R52 · A-239)`);
      setCollapsed((c: any) => ({ ...c, features: false }));
      cursoSet(10, [
        'CICLO DEL DADO · estación 10 — EXPULSIÓN (cap 11): los ciclos, no el pipeline',
        `F_eject ${e10.fuerza.fEjectN} N (Eq 11.7-11.8) = ${e10.fuerza.pctClamp} % del clamp · ${e10.fuerza.pctEyectorMaq} % del eyector IM-50`,
        `el ciclo: 4→⌀2.381 VIOLA pared (R34) → 8→⌀1.588 ✓ → recto PANDEA (SF ${punta.sfRectoK2}) → ESCALONADO (R52) SF ${e10.pines.pandeoNuestro.sf} · pines ceden el carril del baffle (A-239)`,
        `juez: ${e10.juez.claros.length} claros medidos, peor ${Math.min(...e10.juez.claros.map((c: any) => c.claroMm)).toFixed(1)} mm · juzgarPines: ${e10.juicioPines.peor}`,
      ], [...moldParts, ...parts]);
    } catch (e) { console.warn('E10_ERR', e); }
  }, [oc, ciclo, moldParts, cursoPart, cursoSet, setDocName, setCollapsed]);
  // ── ESTACIÓN 11: ESTRUCTURA (cap 12) — todo lo perforado rinde cuentas.
  // Las cotas K sobre los barrenos reales + el checklist R90 de 3 veredictos.
  const cicloEstacion11 = useCallback(() => {
    if (!ciclo || ciclo.estacion !== 10 || !ciclo.e2 || !ciclo.e5datums || !ciclo.e8 || !ciclo.e10) return;
    try {
      const d = ciclo.e5datums;
      const e11 = estacion11Dado(ciclo.e2.pkg, d, ciclo.e8.circuito, ciclo.e10);
      const cx = d.ejeX, cy = d.ejeY, zP = d.zPartMm;
      const cotas = [{
        role: 'nucleo', dims: [
          { id: 'e11-ka', label: `K agua-A→cavidad (Eq 12.19) · σ ${e11.barrenos[0].sigmaEsperadaMPa}≪${e11.limites.endurance}`, kind: 'linear', a: [cx - 30, cy - 30, zP + 55], b: [cx - 30, cy + 30, zP + 55], value: e11.barrenos[0].K, measured: e11.barrenos[0].K, ok: true },
          { id: 'e11-kb', label: `K baffle→macho · σ ${e11.barrenos[1].sigmaEsperadaMPa}≪${e11.limites.endurance}`, kind: 'linear', a: [cx, cy, zP + 10], b: [cx, cy, zP + 30], value: e11.barrenos[1].K, measured: e11.barrenos[1].K, ok: true },
          { id: 'e11-r90', label: `R90: sobrepresión ✓ fatiga ✓ flash ✓ (δ ${e11.soporte.deltaMm} < vent ${e11.soporte.ventGapMm})`, kind: 'linear', a: [cx + 25, cy, zP - 20], b: [cx + 25, cy, zP - 5], value: e11.soporte.deltaMm, measured: e11.soporte.deltaMm, ok: e11.r90.flashOk },
        ],
      }] as any;
      setTFill(1); tFillRef.current = 1;
      setCiclo({ ...ciclo, estacion: 11, e11, e3cotas: cotas });
      setCotasOn(true);
      setDocName(`EL DADO · estación 11 — ESTRUCTURA: R90 ✓✓✓ (sobrepresión · fatiga · flash) — el acero perforado AGUANTA`);
      setCollapsed((c: any) => ({ ...c, features: false }));
      cursoSet(11, [
        'CICLO DEL DADO · estación 11 — ESTRUCTURA (cap 12): lo perforado rinde cuentas',
        `soporte ${e11.soporte.tMm} mm: δ ${e11.soporte.deltaMm} < ${e11.soporte.ventGapMm} (EL VENT DE LA E7) — la alarma maestra R70, verde · 0 pilares (la placa aguanta sola)`,
        `K de los barrenos (Eq 12.19): agua-A ${e11.barrenos[0].K} · baffle ${e11.barrenos[1].K} · pin ${e11.barrenos[2].K} (R81: la grieta se frena en el pin; el riesgo es binding)`,
        `hoop del macho ${e11.hoop.sigmaEsperada}/${e11.hoop.sigmaSobre} MPa · R90: 3 veredictos ✓ · la E3 NO se reabre (soporte ${e11.retornoE3.soporteStackMm} == ${e11.retornoE3.soportePkgMm})`,
      ], moldParts);
    } catch (e) { console.warn('E11_ERR', e); }
  }, [ciclo, moldParts, cursoSet, setDocName, setCollapsed, setCotasOn]);
  // ── ESTACIÓN 12: EL ACTA (§13.10) — no calcula, COBRA. R92: decisiones
  // aprobadas y documentadas con costos/beneficios/riesgos. El cubo se firma.
  const cicloEstacion12 = useCallback(() => {
    if (!ciclo || ciclo.estacion !== 11 || !ciclo.e2 || !ciclo.e5datums) return;
    try {
      const acta = estacion12Dado(ciclo.e2.pkg, ciclo.e5datums);
      // EL ACTA SE LEE EN LA ESCENA: el registro del curso es texto tenue de 11 px
      // (ojo de ian sobre la sonda: "gris sobre gris"). Las tres líneas que COBRAN
      // se rotulan con el MISMO mecanismo de cotas de la E11 — reemplazan a las de
      // estructura, que ya rindieron su veredicto en su propio video.
      const d12 = ciclo.e5datums;
      const cx12 = d12.ejeX, cy12 = d12.ejeY, zP12 = d12.zPartMm;
      const cotas12 = [{
        role: 'nucleo', dims: [
          { id: 'e12-firma', label: `ACTA §13.10 — ${acta.veredicto}: ${acta.conteo.estaciones} estaciones · ${acta.conteo.checksGate} checks · ${acta.retornosCerrados.length} retornos CERRADOS`, kind: 'linear', a: [cx12 - 30, cy12 - 30, zP12 + 55], b: [cx12 - 30, cy12 + 30, zP12 + 55], value: '', measured: null, ok: acta.veredicto === 'FIRMADO' },
          { id: 'e12-arq', label: `LA ARQUITECTURA (E2→E8): ${acta.decisiones[0]?.decision ?? '—'}`, kind: 'linear', a: [cx12, cy12, zP12 + 10], b: [cx12, cy12, zP12 + 30], value: '', measured: null, ok: true },
          { id: 'e12-tryout', label: `TRYOUT steel-safe R119: ${acta.tryout.length} pasos · ${acta.erratas.length} erratas del libro cazadas · ${acta.pendientes.length} pendientes DECLARADOS`, kind: 'linear', a: [cx12 + 25, cy12, zP12 - 20], b: [cx12 + 25, cy12, zP12 - 5], value: '', measured: null, ok: true },
        ],
      }] as any;
      setTFill(1); tFillRef.current = 1;
      setCiclo({ ...ciclo, estacion: 12, e12: acta, e3cotas: cotas12 });
      setCotasOn(true);
      setDocName(acta.veredicto === 'FIRMADO'
        ? `EL DADO · estación 12 — EL ACTA: FIRMADO ✍ (${acta.decisiones.length} decisiones · ${acta.retornosCerrados.length} retornos cerrados · ${acta.conteo.checksGate} checks) — EL CUBO ESTÁ CERRADO`
        : `EL DADO · estación 12 — EL ACTA: INCOMPLETO (faltan ${acta.faltantes.join(', ')})`);
      setCollapsed((c: any) => ({ ...c, features: false }));
      cursoSet(12, [
        'CICLO DEL DADO · estación 12 — EL ACTA (§13.10): el cubo, firmado',
        `ARQUITECTURA: ${acta.decisiones[0]?.decision ?? '—'}`,
        `${acta.retornosCerrados.length} retornos cerrados · ${acta.erratas.length} erratas del libro cazadas · tryout steel-safe de ${acta.tryout.length} pasos`,
        `${acta.conteo.oraculosImpresos}`,
      ], moldParts);
    } catch (e) { console.warn('E12_ERR', e); }
  }, [ciclo, moldParts, cursoSet, setDocName, setCollapsed, setCotasOn]);
