...
const SUITES = [
  ...
  { group: 'kernel', n: 'mold-seccion', why: 'L5 LA SECCIÓN MADRE por el eje del sprue (Fig 1.6): 11 verificaciones, y L6/L7 reusan su máquina de cortes. Verifica contra geometría ANALÍTICA: cilindro por su eje = 2·r·h exacto, cono truncado = h·(r0+r1), vaso no convexo = resta de rectángulos, convergencia 2.67 %→0.008 % al refinar, y una caja partida en 5 da la MISMA área y el MISMO perímetro visible que la entera. Destapa tres bugs reales del motor: insertDims con un +22 fijo en vez de 3·⌀ de la línea ruteada, la mejilla dimensionada con el ⌀ estimado mientras el circuito rutea otro, y un Math.round donde el piso de Eq 9.22 exige redondeo hacia arriba',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/mold-seccion-test.cjs'] },
  { group: 'kernel', n: 'mold-apertura-l6', why: 'L6 LA SECUENCIA DE APERTURA: la única lámina que valida correderas, núcleos móviles y auto-degating, porque barre la INTERFERENCIA a lo largo de TODO el recorrido, no solo en las poses dibujadas. Invariantes: área conservada en las 4 poses (cuerpo rígido, error 0), la corredera avanza d·tan(φ) exacto, carrera = H_pieza + colada en forma cerrada. Cada mecanismo lleva control POSITIVO y NEGATIVO (pin +0.05 mm penetra el área exacta; a ras no da falso positivo). Halla dos defectos reales del molde que genera la Máquina: la carrera de expulsión no cabe en el housing (pide 57, hay 31) y el pin contorneado de §11.2.5 cruza la línea de agua de §9.2',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/mold-apertura-l6-test.cjs'] },
  { group: 'kernel', n: 'mold-compuerta', why: 'L7 EL DETALLE DE LA COMPUERTA: once verificaciones de §6-§7. El orden de eficiencia de perfiles NO se copia de la Tabla 6.3, EMERGE del cociente isoperimétrico Q=4πA/P² y reproduce el ranking de §6.5.1. El reverse taper se juzga por SIGNO contra el bebedero (Fig 7.4 "inverted sprue"), nunca por magnitud. Incluye G-cotas, un invariante sobre las COORDENADAS DEL DIBUJO: el modelo puede estar perfecto y el dibujo mentir — las cotas de ⌀ se cruzaban y la lámina decía que el cono iba al revés, que es justo el veredicto que existe para dar',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/mold-compuerta-test.cjs'] },
  { group: 'kernel', n: 'mold-nucleo-enfriamiento', why: 'L9 EL NÚCLEO ESBELTO Y SU DISPOSITIVO (Tabla 9.3): solver axisimétrico (r,z) verificado contra física CERRADA — conducción axial ⇒ perfil lineal (1.2e-14), cilindro radial ⇒ logarítmico (1.0e-14), pin con flujo lateral ⇒ solución 2D exacta (7.1e-13), conservación y el 4× de §9.3.6 por bisección sobre la serie de Fourier. El veredicto EMERGE: núcleo macizo ΔT 33.8 °C vs 5.07 con baffle. Y separa lo que la tabla PIDE de lo que está DIBUJADO: un núcleo macizo se titulaba "Baffle" con ✓ verde solo porque a ese ⌀ toca baffle',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/mold-nucleo-enfriamiento-test.cjs'] },
  { group: 'kernel', n: 'mold-particion-angulo', why: 'L11 LA PARTICIÓN POR ÁNGULO (regla de 5°, §4.1.3), heredando la visibilidad de L21 para V4.6. Escala FIJA anclada al 5° literal: auto-escalar por percentiles destruiría el umbral. Invariantes cerrados: cono de semiángulo α ⇒ cada faceta = atan(tanα·cos(π/n)) a 1e-12, plano a 5.000° CUMPLE y a 4.900° reprueba 100 %, invariancia de rotación. Cazó que promediar la normal en un quiebre de 90° metía una cuña con ángulo falso: el área roja del bezel pasó de 0.67 % a 7.52 % (el valor exacto de la descomposición) — la lámina subreportaba el defecto 11 veces',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/mold-particion-angulo-test.cjs'] },
  { group: 'kernel', n: 'mold-layflat', why: 'L15 EL MÉTODO A MANO DE KAZMER (§5.5.4 Fig 5.15-5.19): desdoblar, arcos y phantom gates. Reproduce los 280 y 210 mm de Fig 5.18 desde la geometría (error 0.000 %), NO los cita; el cruce race/no-race por bisección da H*=W/2 exacto e independiente de L, así que la razón textual del libro sale sola. Conservación de área al desdoblar 0.00e+0 e isometría 2.8e-14. La prueba de §5.5.5: brazo delgado CERCA vs grueso LEJOS — la distancia y la resistencia se contradicen y gana la resistencia',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/mold-layflat-test.cjs'] },
  { group: 'kernel', n: 'mold-vonmises', why: 'L19 VON MISES EN SECCIÓN (§12.1.1): solver 2D Q4 en deformación plana con recuperación SPR, verificado contra ELASTICIDAD ANALÍTICA — uniaxial 9.9e-15, cortante √3·τ 1.2e-14, hidrostática 2.1e-15, patch test 2.1e-15 y KIRSCH Kt=3.0 medido en 3.00264 (0.088 %, orden 2.46). Reproduce la asimetría de V12.2: el lado móvil flexiona 14× más que el fijo. Y NO finge que la singularidad converge: las esquinas vivas van de 556 a 633 MPa al refinar, se apartan en discos declarados, y el gate verifica LAS DOS COSAS — que el campo bueno converja y que la esquina NO',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/mold-vonmises-test.cjs'] },
  { group: 'kernel', n: 'verif-sdf-malla', why: 'SDF 3D VERDADERO de la malla: un campo que cambia de signo en la superficie NO es un SDF. El invariante que manda es la ECUACIÓN EIKONAL |∇φ| = 1 (falla solo en el eje medial), y es lo que separa una distancia real de una aproximación que se ve bien. Verificado contra fórmula cerrada: esfera |p−c|−R a 7.2e-3 mm y caja (incluidas ARISTAS y ESQUINAS) a 2.8e-7. La medición que justificó el cambio: el sdf 3D da |∇φ|−1 = 1.66e-3 y la aproximación de LOSA EN Z que se usaba antes da 1.08 — 650× peor, porque en las paredes laterales no era una distancia en absoluto. Con su punto ciego MEDIDO y declarado: el signo sale de la paridad de cruces en z, así que un agujero en cara HORIZONTAL se caza (383 columnas impares) y uno en cara VERTICAL es invisible — quien valida cierre es verificacion/matricula.ts',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/verif-sdf-malla-test.cjs'] },
  { group: 'kernel', n: 'verif-termico-cortado', why: 'CELDA CORTADA EN PRODUCCIÓN (thermal-steady): en el solver del molde la frontera crítica no es Dirichlet sino la INTERFAZ MATERIAL plástico↔acero. Barra de dos capas con la interfaz FUERA de cara y FUERA de centro (los dos sitios donde cortada ≡ escalonado), contra referencia de malla fina. El hallazgo que ordenó el arreglo: el error medido (1.808/1.791/1.779/1.656/0.634 %) lo predice EXACTO la cuantización del volumen de plástico (1.816/1.816/1.816/1.690/0.638 %), no la posición de la interfaz — así que arreglar solo la conductancia no movía nada. Con la fracción de volumen también cortada: 25-33× más exacta. Dos controles: sin sdfMm no se activa, y con la interfaz justo en la cara es bit a bit idéntico al escalonado',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/verif-termico-cortado-test.cjs'] },
  { group: 'kernel', n: 'verif-celda-cortada', why: 'CELDA CORTADA (Shortley-Weller): el arnés de MMS midió que cuando el dato de frontera viene de la SUPERFICIE REAL y se impone en la cara del vóxel, el esquema pierde su orden — la CF queda desplazada hasta h/2 y el patrón de escalones CAMBIA con cada malla. El experimento decisivo es una ESFERA fuera de rejilla (ninguna cara se alinea): mismo MMS, misma malla, misma fuente, y lo único que cambia es DÓNDE se cree que está la superficie. Escalonado p=1.29, celda cortada p=1.99 y 35× más exacta en la malla fina. Con control: en un CUBO (frontera alineada) declarar dReal=h/2 no cambia un solo bit, así que la extensión no altera el caso que ya funcionaba',
    cmd: NODE, args: ['--import', 'tsx', 'scripts/verif-celda-cortada-test.cjs'] },
