...
(async () => {
  ...
  if (process.env.PROBETA === '1') {
    ...
    if (process.env.E5 === '1' || process.env.E6 === '1' || process.env.E7 === '1' || process.env.E8 === '1' || process.env.E9 === '1') { await p.waitForTimeout(2500); await clic('btn-ciclo-e5'); }
    ...
    if (process.env.E6 === '1' || process.env.E7 === '1' || process.env.E8 === '1' || process.env.E9 === '1') { await p.waitForTimeout(16000); await clic('btn-ciclo-e6'); }
    // E7 (cap 8): el VENTEO — la E6 es instantánea; el botón e7 vive en su panel
    if (process.env.E7 === '1' || process.env.E8 === '1' || process.env.E9 === '1') { await p.waitForTimeout(2500); await clic('btn-ciclo-e7'); }
    // E8 (cap 9): el AGUA — el botón e8 vive en el panel de la E7
    if (process.env.E8 === '1' || process.env.E9 === '1') { await p.waitForTimeout(2500); await clic('btn-ciclo-e8'); }
    // E9 (cap 10): la CONTRACCIÓN — 3 splitMold en el click (~8 s); el botón vive en CicloE8
    if (process.env.E9 === '1') { await p.waitForTimeout(2500); await p.$eval('[data-testid="btn-ciclo-e9"]', (e) => { setTimeout(() => e.click(), 30); }); await p.waitForTimeout(12000); }
  }
