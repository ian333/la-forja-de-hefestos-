...
(async () => {
  ...
  if (process.env.PROBETA === '1') {
    ...
    if (process.env.E5 === '1' || process.env.E6 === '1') { await p.waitForTimeout(2500); await clic('btn-ciclo-e5'); }
    ...
    if (process.env.E6 === '1') { await p.waitForTimeout(16000); await clic('btn-ciclo-e6'); }
  }
