...
SPECIAL['fisica-cargas gauss'] = {
  ...
  descripcion: `Una carga sola manda sesenta líneas al infinito. Le acercas su opuesta y ninguna se escapa: las sesenta mueren en el menos. ⚡ Eso es la ley de Gauss, y aquí no está dibujada: está CONTADA. Cada línea se traza integrando el campo de verdad, y el programa las cuenta cuadro por cuadro — con seis cargas alternadas en un hexágono son ciento ochenta líneas y CERO fugas, porque la carga neta es cero. Lo que sale de una superficie cerrada solo depende de lo que hay adentro; lo de afuera no cuenta. Pero esas son bolitas ideales… ¿y la materia de verdad? Al final entra un átomo de hidrógeno REAL: un protón y su electrón, que no es una bolita sino una nube de probabilidad. El átomo completo es neutro, así que sus líneas no mueren en ningún lado: SE APAGAN. A dos angstroms ya casi no queda campo — la nube se comió el noventa y ocho por ciento. La misma ley, ahora en materia real. Nada está inventado.\n\n🎓 GAIA University.`,
