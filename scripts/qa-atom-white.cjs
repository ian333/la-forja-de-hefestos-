...
const W = 1540, H = 860;
const OUT = path.join(path.resolve(__dirname, '..'), 'dist-video', '.qa-atomwhite');
...
(async () => {
  ...
  const page = await (await browser.newContext({ viewport: { width: W, height: H }, deviceScaleFactor: 2, bypassCSP: true })).newPage();
  ...
  await browser.close();
  log('listo → dist-video/.qa-atomwhite/');
