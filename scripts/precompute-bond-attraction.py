...
for ki in range(K):
    R=float(Rvals[ki])
    mol=gto.M(atom=[[EL,(-R/2,0,0)],[EL,(R/2,0,0)]],basis=BASIS,spin=0,unit='Bohr',verbose=0)
    ...
    occ=mf.mo_occ; homo=np.where(occ>1e-6)[0][-1]     # sigma2s enlazante = el PAR compartido
    Ch=mf.mo_coeff[:,homo]; dm_bond=2.0*np.outer(Ch,Ch)
    def E2d(P):                                        # campo de los ELECTRONES − del enlace → converge al centro (ATRACCIÓN)
        Q=np.zeros((len(P),3)); Q[:,:2]=P; h=0.03
        def Vb(pts):
            Vm=mol.intor('int1e_grids',grids=np.ascontiguousarray(pts))
            return -np.einsum('gij,ij->g',Vm,dm_bond)  # potencial del − (electrones del enlace)
...
OUT=os.path.join(os.path.dirname(__file__),'..','public','precomputed',f'{MOL}-attraction.bin')
...
p=os.path.join(os.path.dirname(__file__),'..','_o2_proof',f'{MOL}-attraction-verify.png')
